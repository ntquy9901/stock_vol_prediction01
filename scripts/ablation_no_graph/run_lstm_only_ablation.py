"""
LSTM-only ablation for the Parallel LSTM-GNN architecture.

Goal
----
Fill the missing (no graph, no news) cell of the paper's 2x2 ablation grid:
    (graph on/off) x (news on/off).
This isolates the most basic question — does the per-stock LSTM's temporal
nonlinearity alone beat classical HAR (linear regression) — BEFORE adding any of
the architecture's other components (cross-stock graph, news fusion).

The "LSTM-only" model is exactly the price-only backbone's temporal stream: the
same ``ParallelLSTMGNN`` (``src/lstm_gat_hybrid/model_parallel.py``) trained by
``src/lstm_gat_hybrid/train_parallel_enhanced.py`` (no news branch anywhere in
that pipeline), with the GAT/spatial branch's cross-stock message passing
disabled by feeding an IDENTITY adjacency on every forward call. With an identity
adjacency the GAT layers can only attend to each node's own features (off-diagonal
...
LSTM temporal stream is the only cross-anything-carrying pathway. This is the same
identity-adjacency trick, verified the same way, as
``scripts/ablation_no_graph/run_no_graph_ablation.py``.

Note (2026-08-08): ``run_no_graph_ablation.py`` already applies this exact
identity substitution to this exact news-free backbone, so this LSTM-only run is
architecturally a reproduction of it. It is kept as a separate, clearly-named
artifact for the paper's 2x2 grid and as an independent reproduction; see
``docs/reports/2026-08-08_lstm_only_ablation_results.md`` for the grid and the
grid-label clarification.

Isolation
---------
NEW, self-contained script. It does NOT modify model_parallel.py,
train_parallel_enhanced.py, run_no_graph_ablation.py, dataset_with_graph_method.py
or any shared code — it only READS/imports them. The adjacency substitution happens
locally inside this script's own training/validation loops, immediately after each
batch is unpacked and before ``model(x, adj_matrix)`` is called.

Run
---
    python scripts/ablation_no_graph/run_lstm_only_ablation.py --seeds 42 123 2026
    python scripts/ablation_no_graph/run_lstm_only_ablation.py --seeds 42   # single-seed fallback
"""
...
def verify_graph_removed(model, dataloader, device):
    ...
    print("\n" + "=" * 78)
    print("SANITY CHECK: identity-adjacency substitution (LSTM-only)")
    print("=" * 78)
...
def train_epoch(model, dataloader, criterion, optimizer, device, config, verbose_first=False):
    ...
    for x, adj_matrix, y, _ in dataloader:
        x = x.to(device)
        adj_matrix = adj_matrix.to(device)
        adj_matrix = make_identity_adj(adj_matrix)  # <-- LSTM-ONLY (no graph)
        y = y.to(device)
...
def run_one_seed(seed, run_sanity_check=False):
    """Train one seed of the LSTM-only (identity adjacency, no news) ablation."""
    print("=" * 80)
    print(f"LSTM-ONLY ABLATION (identity adjacency, no news) - seed {seed}")
    print("=" * 80)
    ...
    # Dataloaders: IDENTICAL call to the price-only backbone run (graph_method='knn').
    ...
    results_dir = PROJECT_ROOT / f"results/lstm_only_ablation_seed{seed}_{timestamp}"
    ...
    for epoch in range(config.num_epochs):
        t0 = time.time()
        epoch_num = epoch + 1
        print(f"\n[Epoch {epoch_num}/{config.num_epochs}]", flush=True)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), results_dir / "best_lstm_only_model.pth")
    ...
    model.load_state_dict(torch.load(results_dir / "best_lstm_only_model.pth"))
    test_loss, test_metrics = validate(model, test_loader, criterion, device, datasets[2])

    print("\nTest Results (LSTM-only / identity adjacency, no news):")
    for k in ["mse", "rmse", "mae", "r2", "qlike", "directional_accuracy"]:
        print(f"  {k}: {test_metrics[k]:.6f}")

    results = {
        "model": "Parallel LSTM-GNN (LSTM-ONLY ablation: identity adjacency, no news)",
        "timestamp": timestamp,
        "seed": seed,
        "architecture": "Parallel (LSTM temporal + GNN spatial) -> Concatenation fusion",
        "graph_method": "identity (no cross-stock edges) — LSTM-only ablation of price-only backbone",
        "ablation": {
            "type": "lstm_only_identity_adjacency_no_news",
            "reference_run": "graph_method='knn', train_parallel_enhanced.py, 20 epochs (price-only backbone)",
            "description": (
                "Same architecture/data/hyperparameters/protocol as the news-free price-only "
                "backbone; adjacency replaced with torch.eye at every forward call so the GAT "
                "branch cannot pass messages across stocks (per-node transform only, same "
                "parameter count). No news branch anywhere. Fills the (no graph, no news) cell."),
            "sanity_check": sanity,
        },
        "config": {
...
def main():
    parser = argparse.ArgumentParser(description="LSTM-only (identity adjacency, no news) ablation")
    ...
    print("\n" + "=" * 80)
    print("LSTM-ONLY ABLATION SUMMARY")
    print("=" * 80)
