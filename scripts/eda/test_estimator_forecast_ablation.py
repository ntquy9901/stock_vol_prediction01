...
def test_write_estimator_processed_is_date_sorted_and_deduped(tmp_path, monkeypatch):
    # External review H-04/R-07: raw with shuffled + duplicate dates must not corrupt the order-dependent
    # rolling/ewm estimators nor the (date, value) pairing. The written processed CSV must be sorted,
    # unique-by-date; and for a duplicate date with DIFFERENT values, keep="last" + STABLE sort must
    # deterministically keep the row that appeared LAST in the original file.
    proc, raw = _synth(tmp_path, tickers=("AA",), n=300)
    rf = raw / "AA_ohlcv.csv"
    clean = pd.read_csv(rf)
    dup_date = clean.loc[100, "date"]
    last_dup = clean.loc[[100]].copy()
    last_dup["high"] = clean.loc[100, "close"] + 5.0          # distinct, wider range -> distinct Parkinson
    dup = pd.concat([clean, last_dup], ignore_index=True)     # the DISTINCT dup row appears LAST in the file
    dup.to_csv(rf, index=False)
    monkeypatch.setitem(AB.PROC, "synthetic", proc)
    monkeypatch.setitem(VE.PRICE, "synthetic", raw)
    out = tmp_path / "out"; out.mkdir()
