...
The ``parkinson_variance`` column is a VARIANCE (sigma^2 = ln(H/L)^2 / (4 ln2)), NOT sigma -- every
...
def scan_ticker(raw_path: str, proc_path: str | None) -> dict:
    ...
    if proc_path and Path(proc_path).exists():
        pk = pd.to_numeric(_read_sorted(proc_path).get("parkinson_variance"), errors="coerce").to_numpy(float)
...
def market_pk_series(panel: str) -> pd.Series:
    ...
    for pf in sorted(glob.glob(str(proc_dir / "*_processed.csv"))):
        ...
        df = _read_sorted(pf, cols=["date", "parkinson_variance"])
        if len(df):
            cols[tk] = pd.Series(np.sqrt(pd.to_numeric(df["parkinson_variance"], errors="coerce")
...
def render_html(rep: dict, out_path: str) -> str:
    ...
    P.append("<p class='note'>Source: Yahoo Finance daily OHLCV (split/dividend adjusted &mdash; verified: AAPL "
             "2020-08-31 4:1 split shows no price discontinuity). The <code>parkinson_variance</code> target "
    ...
    pk_series = pd.to_numeric(rp["parkinson_variance"], errors="coerce").to_numpy(float)
...
def render_md(rep: dict, out_path: str) -> str:
    ...
    L = ["# S&P 500 EDA & data mining - key findings", "",
         f"Source: Yahoo Finance daily OHLCV (split/dividend adjusted). Target `parkinson_variance` is a "
