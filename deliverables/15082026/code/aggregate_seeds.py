"""Aggregate the trackA ablation across seeds: per-rung mean +/- std of each metric, per horizon.

Reads results/trackA_ablation_h{h}_seed{s}_<TS>/ladder_metrics.json (rungs HAR/FULL/minus_*) and
...
def _run_dir(ts: str, horizon: int, seed: int) -> Path:
    return ROOT / "results" / f"trackA_ablation_h{horizon}_seed{seed}_{ts}"
