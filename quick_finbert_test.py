...
from src.sentiment.models.finbert_sentiment import FinBERTSentiment
...
def test_finbert_on_stocks() -> dict:
    ...
    try:
        ...
        print("💡 Please install dependencies: pip install transformers torch")
        return None

    # Get sample news
    print("\n📰 Loading sample news data...")
    sample_news = get_sample_news()
    print(f"✅ Loaded {sum(len(news) for news in sample_news.values())} news articles")
    ...
    for ticker, news_list in sample_news.items():
        ...
        for i, news_text in enumerate(news_list, 1):
            try:
                ...
                print(f"  Article {i}: {label_emoji} {result.sentiment_label}")
        ...
        if sentiment_scores:
            ...
            print(f"📈 {ticker} Aggregate Sentiment:")
    ...
    if all_sentiments:
        print(f"\n📈 Overall Market Sentiment: {np.mean(all_sentiments):+.3f}")
        print(f"📊 Sentiment Range: {np.min(all_sentiments):+.3f} to {np.max(all_sentiments):+.3f}")
        print(f"📉 Standard Deviation: {np.std(all_sentiments):.3f}")
...
def save_results_to_csv(results: dict, output_dir: str = None):
    ...
    for ticker, ticker_results in results.items():
        ...
        print(f"  ✅ Saved: {ticker}_sentiment_daily.csv ({len(df)} articles)")
...
def main():
    ...
    if results:
        ...
        print("=" * 60)
        print("\n✅ FinBERT is working for VN30 stocks!")
        print("✅ Sentiment analysis pipeline validated!")
        print("✅ CSV data files generated!")
        print("\n📂 Check results in: data/processed/vn30_sentiment/daily/")
        print("🚀 Ready to scale to all 30 VN30 stocks!")
        print("=" * 60)
