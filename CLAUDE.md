...
- **Leakage test phải ở RANH GIỚI feature-consumption, không chỉ hàm thuần (ENFORCED, per root-cause 2026-09-19).** Leakage là thuộc tính của TOÀN pipeline: một helper causal đã unit-test (vd `predict_schedule` sửa `gaps[:i-1]`) VẪN leak vì vùng khởi tạo/anchor/warmup của nó (3 anchor = ngày ACTUAL) bị 1 feature nhìn-tới-trước tiêu thụ ở `full_matrix.panel`. Bắt buộc: (1) test tại chỗ feature **tiêu thụ** schedule/state (panel/dataset), đặt forecast origin/target TRƯỚC khi đủ lịch sử, assert feature forward = neutral; (2) soi mọi vùng warmup/anchor/seed/first-fold — giá trị raw ở đó có bị đọc như thông tin TƯƠNG LAI không (past-actual OK, future-actual = leak); (3) test phải **phân biệt** được causal≠leaky (chọn input mà 2 cái khác nhau) + có contrast case feed bản leaky để test fail nếu regress; (4) blast-radius theo date-range dữ liệu vs train/test fold. `/code-review` phải có lăng kính này cho mọi feature dùng ngày/nhãn tương lai. Ref: `baselines/2026-09-19_expected_schedule/code_review/code_review_2026-09-19.md`.
