...
1. **Constitution** — Đã có sẵn: chính file `AGENTS.md` này là constitution của project (nguyên tắc,
   tiêu chuẩn, gate). Spec/plan mới phải tuân theo rule đã có ở đây; conflict thì AGENTS.md thắng —
   sửa AGENTS.md trước rồi mới sửa spec.
...
- **Lint scope:** exclude vendored/generated/third-party (`.agents .Codex _bmad archive data`).
...
> Nguồn: mượn từ project sibling `thesis/data_eda` (AGENTS.md), áp dụng theo yêu cầu user 2026-07-15.
...
- **Lint excludes:** `.agents .Codex _bmad archive data` (khớp `ruff.toml` `extend-exclude`)
- **Smoke command:** `python -m pytest -m smoke` (marker `smoke` đã register trong `pytest.ini`)
- **Code-review tool:** `/code-review` skill (Codex) — chỉ gọi được qua user gõ trực tiếp `/code-review`, KHÔNG gọi được qua Skill tool trong 1 agent session (`disable-model-invocation`) — nếu cần review trong session, tự làm adversarial review thủ công + `ReportFindings`.
...
└── YYYY-MM-DD_<tên-baseline>/      ← timestamp = ngày bắt đầu implement
    ...
    ├── code_review/
    │   └── code_review_<YYYY-MM-DD>.md  # Kết quả review (adversarial, per AGENTS.md mục 5)
    └── test/
        ├── __init__.py
