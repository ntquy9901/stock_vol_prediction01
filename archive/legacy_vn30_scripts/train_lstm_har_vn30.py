"""
Train LSTM-HAR on VN30-only data
"""
import sys
import torch
import numpy as np
...
print('='*80)
print('LSTM-HAR - VN30-ONLY TRAINING')
print('='*80)

# Create result directory
output_dir = Path('results/lstm_har_vn30_2026-06-20')
...
# windows of every ticker into train. See HARVolatilityDataset's split= param.
print('\n1. Creating VN30 dataset (temporal split)...')
torch.manual_seed(42)
np.random.seed(42)
train_dataset = HARVolatilityDataset(data_dir, seq_length=seq_length,
                                     forecast_horizon=forecast_horizon, split='train')
val_dataset = HARVolatilityDataset(data_dir, seq_length=seq_length,
                                   forecast_horizon=forecast_horizon, split='val',
                                   feature_scaler=train_dataset.feature_scaler,
                                   target_scaler=train_dataset.target_scaler)
test_dataset = HARVolatilityDataset(data_dir, seq_length=seq_length,
...
model = HARVolatilityLSTM(hidden_size=hidden_size, num_layers=num_layers, dropout=dropout)
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        best_epoch = epoch
        epochs_no_improve = 0
        torch.save(model.state_dict(), output_dir / 'best_lstm_har_vn30.pth')
    else:
        epochs_no_improve += 1
...
# Evaluate
print('\n4. Evaluating...')
model.load_state_dict(torch.load(output_dir / 'best_lstm_har_vn30.pth'))
model.eval()
...
print('\n' + '='*80)
print('LSTM-HAR (VN30-ONLY) RESULTS')
print('='*80)
...
print(f'  LSTM-HAR Dir Acc: {metrics["directional_accuracy"]:.2f}%')
print(f'  Improvement: {metrics["directional_accuracy"] - 51.53:.2f}%')

# Save results
import json
results = {
    'model': 'LSTM-HAR (VN30-Only)',
