"""Train the pure-market baseline (HAR + broadcast market-news branch, no ticker matching).

Mirrors the sibling 2026-07-07_embedding_baseline's train_embedding_baseline.py structure
(train_epoch/validate/EarlyStopping/plot/results.json), adapted for the 5-tuple batch
(x_har, adj, x_market, market_mask, y) and PureMarketBaseline.forward's 4-arg signature.
No edits to any sibling baseline.

Run (smoke, no market cache needed — dataset falls back to dummy emb_dim=64):
  python baselines/2026-07-15_pure_market_baseline/code/train_pure_market.py --epochs 2 --smoke

Real run (data/sentiment_embedding/market_emb.npz already exists, from sibling baseline):
  python baselines/2026-07-15_pure_market_baseline/code/train_pure_market.py --epochs 10
"""
import sys
import argparse
import json
...
from dataset_pure_market import create_pure_market_dataloaders


def train_epoch(model, loader, criterion, optimizer, device, grad_clip=1.0):
    model.train()
    total, nb = 0.0, 0
    for x_har, adj, x_market, market_mask, y in loader:
        x_har = x_har.to(device); adj = adj.to(device); x_market = x_market.to(device)
        market_mask = market_mask.to(device); y = y.to(device)
        B, S = y.shape
        optimizer.zero_grad()
        pred = model(x_har, adj, x_market, market_mask)   # [B, S]
...
def main():
    ...
    ap.add_argument("--market_cache_path", default=str(_ROOT / "data" / "sentiment_embedding" / "market_emb.npz"))
    ...
    ap.add_argument("--smoke", action="store_true",
                    help="quick run (no real market cache needed; verifies shapes/forward)")
    ...
    market_cache_path = args.market_cache_path if not args.smoke else None
    train_loader, val_loader, test_loader, (train_ds, val_ds, test_ds) = \
        create_pure_market_dataloaders(
            data_dir=args.data_dir, market_cache_path=market_cache_path,
            graph_method=args.graph_method, batch_size=args.batch_size, config=config)

    emb_dim = train_ds._emb_dim or args.d_news
    model = PureMarketBaseline(config, emb_dim=emb_dim, d_news=args.d_news,
    ...
    out_dir = _ROOT / "results" / f"pure_market_{timestamp}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ckpt_dir = _ROOT / "models" / f"pure_market_{timestamp}"
