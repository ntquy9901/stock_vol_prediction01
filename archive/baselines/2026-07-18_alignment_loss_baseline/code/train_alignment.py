"""Train the M2VN-style alignment-loss baseline (arXiv:2510.20699).

loss = MSE(pred, y) + lambda_align * (1 - cosine_similarity(proj_har, proj_news)).
Reuses create_embedding_dataloaders (sibling 2026-07-07, read-only import) — same data as the
original embedding baseline (68.76%@40ep), so results are directly comparable.

Run (smoke): python .../train_alignment.py --epochs 2 --smoke
Real run:    python .../train_alignment.py --epochs 15
"""
import sys
import argparse
import json
...
def train_epoch(model, loader, criterion, optimizer, device, lambda_align, grad_clip=1.0):
    model.train()
    total, nb = 0.0, 0
    for x_har, adj, x_emb, mask, y in loader:
        ...
        loss_pred = criterion(pred.reshape(B * S), y.reshape(B * S))
        loss_align = alignment_loss(proj_har, proj_news)
        loss = loss_pred + lambda_align * loss_align
...
def validate(model, loader, criterion, device, dataset):
    ...
    with torch.no_grad():
        for x_har, adj, x_emb, mask, y in loader:
            x_har = x_har.to(device); adj = adj.to(device); x_emb = x_emb.to(device)
            mask = mask.to(device); y = y.to(device)
            pred, _proj_har, _proj_news = model(x_har, adj, x_emb, mask)
...
def main():
    ...
    ap.add_argument("--d_align", type=int, default=32)
    ...
    np.random.seed(42)
    print(f"[train] device={device}, smoke={args.smoke}, lambda_align={args.lambda_align}")
    ...
    model = AlignmentLossBaseline(config, emb_dim=emb_dim, d_news=args.d_news,
                                  d_align=args.d_align, dropout=args.dropout).to(device)
    print(f"[train] model emb_dim={emb_dim}, d_news={args.d_news}, d_align={args.d_align}")
    ...
    out_dir = _ROOT / "results" / f"alignment_{timestamp}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ckpt_dir = _ROOT / "models" / f"alignment_{timestamp}"
    ...
    for ep in range(args.epochs):
        tl = train_epoch(model, train_loader, criterion, optimizer, device, args.lambda_align)
    ...
    results = {
        "model": "AlignmentLossBaseline",
        "emb_dim": int(emb_dim), "d_news": args.d_news, "lambda_align": args.lambda_align,
