"""Train the Ablation-Derived News Gate baseline (HAR + news branch, masked to the 11 tickers
whose news-vs-no-news delta_QLIKE was derived from this project's own LSTM-GNN, not borrowed
from a different model family).

Reuses `create_dual_news_dataloaders` from `2026-07-25_dual_group_news_embedding_baseline`
(read-only, no data rebuild — same dual_group_news_panel.parquet). Mirrors
`2026-07-25_selective_news_gate_baseline/code/train_selective_gate.py`, swapping in
`AblationDerivedGateBaseline` (11-ticker allowlist from `2026-07-25_news_usefulness_ablation`).

Training policy (CLAUDE.md): default 10 epochs for this first experimental run.

Run (smoke, no panel needed):
  python baselines/2026-07-25_ablation_derived_gate_baseline/code/train_ablation_gate.py --epochs 2 --smoke
...
for _p in (str(_ROOT), str(_CODE), str(_SIBLING_DUAL_CODE), str(_SIBLING_GATE_CODE)):
...
from model_ablation_gate import NEWS_ON_TICKERS, AblationDerivedGateBaseline
...
def _per_stock_dir_acc(preds_d, targs_d, stock_names):
    """Per-ticker directional accuracy + NEWS_ON(3)/NEWS_OFF(rest) group averages."""
    ...
    off_vals = [v for k, v in per_ticker.items() if k not in NEWS_ON_TICKERS]
...
def main():
    ...
    model = AblationDerivedGateBaseline(config, n_feat=n_feat, stock_names=train_ds.stock_names,
    ...
    print(f"[train] model n_feat={n_feat}, d_news={args.d_news}, "
          f"NEWS_ON stocks={n_on}/{len(train_ds.stock_names)} ({sorted(NEWS_ON_TICKERS)})")
    ...
    out_dir = _ROOT / "results" / f"ablation_gate_{timestamp}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ckpt_dir = _ROOT / "models" / f"ablation_gate_{timestamp}"
    ...
    for ep in range(args.epochs):
        ...
        print(f"{ep+1:>4} | {tl:>11.6f} | {vl:>9.6f} | {vm['directional_accuracy']:>6.2f}% | "
              f"{on_s:>7} | {off_s:>8} | {vm['rmse']:>10.6f}", flush=True)
        if (ep + 1) % 5 == 0:
            print(f"      [5-epoch report] MSE={vm['mse']:.6f} RMSE={vm['rmse']:.6f} "
                  f"MAE={vm['mae']:.6f} R2={vm['r2']:.6f} QLIKE={vm['qlike']:.6f} "
                  f"DirAcc={vm['directional_accuracy']:.2f}% ON(3)={on_s} OFF(29)={off_s}",
                  flush=True)
        if vl < best_vl:
            best_vl = vl
            best_vm = vm
    ...
    results = {
        "model": "AblationDerivedGateBaseline",
        "n_feat": int(n_feat), "d_news": args.d_news, "smoke": bool(args.smoke),
        "news_on_tickers": sorted(NEWS_ON_TICKERS),
