...
% Numbers taken verbatim from soict_2026-09-19_1430_v6_review5.tex. Do NOT invent numbers.
...
\begin{frame}{Bài toán \& động lực}
  \begin{itemize}
    ...
    \item Kiểm định ý nghĩa thống kê: \textbf{Diebold-Mariano} (date-clustered theo ngày giao dịch).
...
\begin{frame}{Dữ liệu \& giao thức}
  \begin{itemize}
    \item Hai thị trường khác biệt cấu trúc:
      ...
      \end{itemize}
    \item Dùng \textbf{daily OHLC} vì \textbf{dễ có, miễn phí} cho cả 2 thị trường (intraday bị tính phí/hạn chế/khó có); Parkinson tái tạo biên độ trong ngày từ high--low nên ít mất mát.
    ...
    \item Embargo tăng theo horizon (chống leakage); floor $10^{-8}$ (giữ QLIKE hữu hạn) và cap $1.0$ áp \textbf{đồng nhất} mọi model.
  \end{itemize}
\end{frame}
...
\begin{frame}{Kết quả 2 --- Earnings: đòn bẩy khiêm tốn (chỉ S\&P~500)}
  \begin{itemize}
    ...
    \item Nguyên nhân HOSE: event study cho thấy \textbf{không có spike ngày công bố} (median $\approx$1.08$\times$ baseline, phẳng), khác hẳn S\&P~500 --- giới hạn biên độ giá VN có thể dập phản ứng.
  \end{itemize}
\end{frame}
...
\begin{frame}{Vì sao? (Discussion)}
  \begin{itemize}
    \item \textbf{Event study earnings}: median variance ngày công bố S\&P~500 tăng $\approx$\textbf{2.75$\times$} baseline; HOSE gần như \textbf{phẳng} ($\approx$1.08$\times$) --- lý giải vì sao earnings chỉ giúp S\&P~500 (giới hạn biên độ giá VN có thể dập phản ứng).
    \item \textbf{Leaf-graph} giúp HOSE (khử nhiễu cross-sectional ở thị trường mỏng) nhưng $\alpha\to0$ trên S\&P~500 (không đóng góp).
    \item Thông điệp: tín hiệu bên ngoài chỉ giúp khi \textbf{khớp cấu trúc thị trường} --- không có cơ chế phổ quát; own-history đã ở frontier QLIKE.
  \end{itemize}
\end{frame}
...
\begin{frame}{Tính trung thực \& Limitations}
  \begin{itemize}
    \item VolTree là \textbf{mô hình hợp nhất}, giữ số gần bằng biến thể tốt nhất theo từng thị trường (không vượt trội thêm).
    \item Earnings dùng \textbf{cadence-estimated} (leak-free); ngày thực tế chỉ cho \emph{upper bound} 8.7--11.2\% --- không point-in-time.
    \item Lịch earnings HOSE: sai số dự đoán ngày median \textbf{3 ngày} (S\&P~500 2 ngày; đuôi nặng hơn) --- lịch \textbf{không nhiễu}; earnings không transfer là do phản ứng ngày công bố bị dập, không phải lịch xấu. Chỉ khẳng định "không có giá trị earnings tăng thêm dưới dữ liệu hiện có", không phải null tuyệt đối.
    \item Chỉ \textbf{2 thị trường}, chỉ daily OHLC (Parkinson) --- có thể không chuyển sang biến động từ giá intraday hoặc từ giá quyền chọn.
    \item Cửa sổ stress (spike-robustness) là kiểm định độ nhạy pre-specified, không phải test chính.
  \end{itemize}
\end{frame}
...
\begin{frame}{Kết luận}
  ...
  \vspace{0.5em}
  \begin{center}
    \textit{Hướng tương lai: mở rộng ngoài 2 thị trường, sang biến động từ giá intraday hoặc từ giá quyền chọn.}
  \end{center}
\end{frame}
