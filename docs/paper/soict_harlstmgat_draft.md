...
On VN30 and VN100, with a pooled model, 80/10/10 splits, five seeds and Diebold–Mariano testing, we
find that **HAR remains the strongest forecaster at both the 1-day and 1-week horizons**, that the
proposed model does not beat it, and that a leave-one-out ablation shows the graph-attention branch
**consistently hurts** accuracy. All learned models beat a GARCH(1,1) baseline. We report the result
honestly, analyse why the deep model underperforms (regime shift under a chronological split;
overfitting of the cross-sectional graph), and discuss the conditions under which deep models are and
are not competitive with HAR.
...
VN30 (33 tickers) and VN100 (104, vnstock source). Common-date snapshots with a fixed node set and a
chronological 80/10/10 split; one pooled model; per-ticker standardisation fit on training rows only;
graphical-lasso Top-5 edges on training rows only. Training: 20 epochs max, early-stop (patience 3),
dropout 0.2, weight decay 1e-5, gradient clipping, ReduceLROnPlateau; five seeds {42,123,2026,7,2024}.
Evaluation: MSE, RMSE, MAE, QLIKE (positivity floor 1e-8, identical across models), R²; seed-averaged;
Diebold–Mariano (HLN, HAC lag h−1) on QLIKE. S&P500 was attempted but excluded: the GAT attention is
O(N²) and exhausts 8 GB VRAM at 500 nodes.
...
For daily Parkinson-volatility forecasting of Vietnamese equities, HAR remains the strongest baseline;
a graph-attention LSTM over a graphical-lasso graph does not improve on it and the graph branch
consistently hurts, though all learned models beat GARCH. The negative, reproducible ablation is the
