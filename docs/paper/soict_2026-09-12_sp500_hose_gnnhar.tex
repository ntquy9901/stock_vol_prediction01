...
% Requires llncs.cls + splncs04.bst (present in docs/paper/).
% Content converted verbatim (all sections, numbers, tables, figures) from
%   docs/paper/2026-09-12_sp500_hose_gnnhar_final.md.
% Bolding rule: bold marks ONLY the QLIKE that is the significant best by a
%   date-clustered Diebold-Mariano test; nothing else is bolded.
\documentclass{llncs}
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{amsmath}
...
\title{Forward-Looking Earnings Dominate Cross-Firm Graphs in Daily S\&P 500 Volatility Forecasting}
...
The graph null replicates on a second market, 405 Vietnam HOSE tickers, where the earnings lever does not transfer,
which localizes the earnings gain to the S\&P 500 while the graph null holds on both markets.
\keywords{Volatility forecasting \and HAR \and Graph neural network \and Earnings \and Diebold--Mariano.}
\end{abstract}

\section{Introduction}
Daily volatility prices options and margin. The Heterogeneous Autoregressive (HAR) model~\cite{corsi2009} forecasts realized
...
\textbf{Target and metric.} We forecast daily Parkinson variance, $\mathrm{pk}_t=\ln(H_t/L_t)^2/(4\ln 2)$~\cite{parkinson1980}, at horizons
$h\in\{1,5,10,22\}$ over 497 S\&P 500 constituents. We report QLIKE~\cite{patton2011} as the primary loss, with MSE, RMSE, MAE, and
$R^2$. We test significance with a Diebold--Mariano statistic~\cite{dm1995} on per-trading-date mean loss, because within-day
stock errors are dependent.

\textbf{Baselines.} HAR fits three own-history lags by ordinary least squares. HARQ adds a realized-quarticity
...
\textbf{Market factor as a graph.} A neighbor aggregate under a uniform adjacency, $g_{\mathrm{mkt}}[i,t]=\frac{1}{N-1}
...
\textbf{Protocol.} We use expanding-window walk-forward folds with a target-horizon embargo. All correlations,
...
\section{Cross-Firm Graphs Add No Incremental Value Beyond Own-History and a Market Factor}
Table~\ref{tab:sp-graph} reports QLIKE for the own-history GBM and its graph-augmented variants on the S\&P 500.

\begin{table}[t]
\centering
\caption{QLIKE, S\&P 500 graph variants. No model is DM-distinguishable from the market aggregate, so no value is
bolded. Bold marks the QLIKE that is significantly best by a date-clustered Diebold--Mariano test; unbolded values
are not DM-distinguishable from the best.}
\label{tab:sp-graph}
\footnotesize
\setlength{\tabcolsep}{6pt}
\begin{tabular}{lccccll}
\toprule
$h$ & GBM (own) & +market & +corr & +placebo & corr vs market & corr vs placebo \\
\midrule
...
\textbf{A sector graph and an explicit conditional test reach the same null.} Table~\ref{tab:sp-sector} adds an industry-sector
adjacency, whose neighbor aggregate averages same-sector firms, and the direct Own+Market+Corr comparison.

\begin{table}[t]
\centering
\caption{QLIKE, S\&P 500 sector and conditional test. No model is DM-distinguishable from the market aggregate, so
no value is bolded. Bold marks the QLIKE that is significantly best by a date-clustered Diebold--Mariano test;
...
\textbf{The null holds on every metric, not QLIKE alone.} Table~\ref{tab:sp-allmetrics} reports QLIKE, RMSE, MAE, and $R^2$ for all seven
...
\begin{table}[t]
\centering
\caption{All metrics, S\&P 500. RMSE and MAE in units of $10^{-4}$. This is a graph-null comparison, so no value is
bolded. Bold marks the QLIKE that is significantly best by a date-clustered Diebold--Mariano test; unbolded values
are not DM-distinguishable from the best.}
\label{tab:sp-allmetrics}
\footnotesize
\setlength{\tabcolsep}{6pt}
\begin{tabular}{llcccc}
\toprule
$h$ & Model & QLIKE & RMSE & MAE & $R^2$ \\
\midrule
1  & HAR      & 0.3697 & 7.586 & 2.132 & 0.158 \\
1  & HARQ     & 0.3780 & 7.515 & 2.151 & 0.174 \\
1  & GBM      & 0.3570 & 7.454 & 2.089 & 0.187 \\
1  & +market  & 0.3539 & 7.423 & 2.075 & 0.194 \\
1  & +corr    & 0.3536 & 7.460 & 2.083 & 0.186 \\
1  & +sector  & 0.3531 & 7.435 & 2.077 & 0.191 \\
1  & +placebo & 0.3546 & 7.437 & 2.076 & 0.191 \\
\midrule
5  & HAR      & 0.4364 & 7.836 & 2.297 & 0.100 \\
5  & HARQ     & 0.4377 & 7.831 & 2.309 & 0.101 \\
5  & GBM      & 0.4116 & 7.821 & 2.255 & 0.103 \\
5  & +market  & 0.4127 & 7.977 & 2.302 & 0.068 \\
5  & +corr    & 0.4108 & 7.882 & 2.279 & 0.090 \\
5  & +sector  & 0.4113 & 7.887 & 2.281 & 0.088 \\
5  & +placebo & 0.4118 & 7.924 & 2.284 & 0.080 \\
\midrule
10 & HAR      & 0.4615 & 7.883 & 2.356 & 0.093 \\
10 & HARQ     & 0.4614 & 7.877 & 2.358 & 0.094 \\
10 & GBM      & 0.4307 & 7.913 & 2.312 & 0.085 \\
10 & +market  & 0.4344 & 7.959 & 2.316 & 0.075 \\
10 & +corr    & 0.4328 & 7.984 & 2.344 & 0.069 \\
10 & +sector  & 0.4316 & 7.973 & 2.337 & 0.072 \\
10 & +placebo & 0.4310 & 8.002 & 2.337 & 0.065 \\
\midrule
22 & HAR      & 0.4855 & 7.954 & 2.419 & 0.079 \\
22 & HARQ     & 0.4831 & 7.944 & 2.415 & 0.081 \\
22 & GBM      & 0.4464 & 7.895 & 2.359 & 0.092 \\
22 & +market  & 0.4605 & 7.877 & 2.369 & 0.096 \\
22 & +corr    & 0.4495 & 7.894 & 2.383 & 0.093 \\
22 & +sector  & 0.4496 & 7.869 & 2.377 & 0.098 \\
22 & +placebo & 0.4489 & 7.860 & 2.376 & 0.100 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{The short-horizon signal is redundant with own-history momentum.} On a minimal three-lag HAR base, the
...
\textbf{An oracle bound locates the missing signal in time.} Replacing the neighbor value at the origin $t$ with the
...
\textbf{Takeaway.} Conditional on own-history and a market factor, cross-firm graph structure adds no reliable
...
\FloatBarrier
\section{Forward-Looking Earnings Are the Lever}
\textbf{A diagnostic locates the missing lever.} The own-history GBM beats HAR-X across the calm
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{figures/fig_remaining_error_by_decile.png}
\caption{Panel B: the own-history GBM beats HAR-X on the calm deciles, and its remaining QLIKE concentrates in the
storm decile D9, which carries 43\% of the model total.}
\label{fig:panelB}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{figures/fig_forecast_bias_by_decile.png}
\caption{Panel C: the GBM under-forecasts the storm deciles, where its median forecast-to-realized ratio drops
below one.}
\label{fig:panelC}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{figures/fig_feature_importance.png}
\caption{Panel E: permutation importance ranks the own-history HAR lags far above every other feature, so
own-history is exhausted.}
\label{fig:panelE}
\end{figure}

Table~\ref{tab:sp-earn} adds a scheduled-earnings feature to the own-history GBM.

\begin{table}[t]
\centering
\caption{QLIKE, S\&P 500 earnings. GBM+earn beats GBM at every horizon (DM $p<0.001$). Bold marks the QLIKE that is
significantly best by a date-clustered Diebold--Mariano test; unbolded values are not DM-distinguishable from the
best.}
\label{tab:sp-earn}
\footnotesize
\setlength{\tabcolsep}{8pt}
...
\midrule
1  & 0.3570 & \textbf{0.3172} & $+11.2\%$ ($p<.001$) & $+0.42\%$ ($p{=}.20$) \\
5  & 0.4116 & \textbf{0.3705} & $+10.0\%$ ($p<.001$) & $-0.26\%$ ($p{=}.46$) \\
10 & 0.4307 & \textbf{0.3912} & $+9.2\%$ ($p<.001$)  & $-0.68\%$ ($p{=}.06$) \\
22 & 0.4464 & \textbf{0.4074} & $+8.7\%$ ($p<.001$)  & $-0.90\%$ ($p<.001$) \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:sp-earn-allmetrics} reports all metrics for the own-history GBM, the earnings model, and the earnings-plus-graph model. The
...
\begin{table}[t]
\centering
\caption{Earnings across all metrics, S\&P 500. RMSE and MAE in units of $10^{-4}$. Bold marks the QLIKE that is
significantly best by a date-clustered Diebold--Mariano test; unbolded values are not DM-distinguishable from the
best.}
\label{tab:sp-earn-allmetrics}
\footnotesize
\setlength{\tabcolsep}{6pt}
\begin{tabular}{llcccc}
\toprule
...
\midrule
1  & GBM           & 0.3570 & 7.454 & 2.089 & 0.187 \\
1  & GBM+earn      & \textbf{0.3172} & 7.346 & 2.017 & 0.211 \\
1  & GBM+earn+corr & 0.3158 & 7.365 & 2.010 & 0.207 \\
\midrule
5  & GBM           & 0.4116 & 7.821 & 2.255 & 0.103 \\
5  & GBM+earn      & \textbf{0.3705} & 7.728 & 2.180 & 0.125 \\
5  & GBM+earn+corr & 0.3715 & 7.787 & 2.196 & 0.111 \\
\midrule
10 & GBM           & 0.4307 & 7.913 & 2.312 & 0.085 \\
10 & GBM+earn      & \textbf{0.3912} & 7.822 & 2.239 & 0.107 \\
10 & GBM+earn+corr & 0.3939 & 7.899 & 2.272 & 0.089 \\
\midrule
22 & GBM           & 0.4464 & 7.895 & 2.359 & 0.092 \\
22 & GBM+earn      & \textbf{0.4074} & 7.809 & 2.277 & 0.112 \\
22 & GBM+earn+corr & 0.4110 & 7.802 & 2.300 & 0.114 \\
\bottomrule
\end{tabular}
\end{table}
...
\FloatBarrier
\section{Recent Graph and Cross-Market Methods Do Not Survive}
We reimplement eight recent methods under a common daily-OHLC information regime and score each against the
baseline: H-ETE-GNN (transfer-entropy country-ETF graph), DCRNN-HAR (dynamic Diebold--Yilmaz spillover~\cite{dy2012}), ASTGCN
...
\textbf{GNNHAR: a learned multi-hop GNN does not exploit cross-firm structure beyond own-history plus a market factor.}
The eighth method is a faithful re-implementation of GNNHAR~\cite{gnnhar} (Zhang, Pu, Cucuringu and Dong, IJF 2024,
...
Several correlation-graph GNN variants show per-seed QLIKE blow-ups (GNNHAR2L-corr-HAR3 per-seed 4.09 $\pm$ 17.8 at
...
and correlation-graph variants blow up (GNNHAR2L-corr-OWN9 ensemble 40.17 at $h1$ with fit\_diagnostics=overfit and
...
notebook and committed under \texttt{results/gnnhar/}.

\section{Related Work}
\textbf{Graph volatility forecasting.} GNNHAR~\cite{gnnhar}, STG-Spillover, and the methods of Section 5 wire stocks or markets
...
\textbf{HAR and its extensions.} HAR~\cite{corsi2009}, VHAR, and HARQ~\cite{harq2016} model volatility from own history and linear spillover. We keep
HAR and HARQ as baselines and show a QLIKE-loss gradient-boosting model with earnings dominates them without a
graph.

\textbf{Earnings and volatility.} Studies document that earnings announcements raise implied and realized volatility.
...
\FloatBarrier
\section{Cross-Firm Graphs and Earnings on the Vietnam Market (HOSE)}
...
\textbf{Setup.} We rebuild the panel on 405 HOSE tickers, forecasting daily Parkinson variance at the same four
...
\textbf{The own-history GBM wins and the graph adds nothing.} GBM posts the lowest QLIKE and the highest $R^2$ at
every horizon (QLIKE $1.5679/1.6478/1.6842/1.7263$, $R^2$ $0.198/0.133/0.107/0.073$), while HAR and HARQ trail by a
wide margin (QLIKE 1.79 to 1.84, $R^2$ near zero to 0.05). Table~\ref{tab:hose-allmetrics} reports all metrics for the eight models. The
...
\begin{table}[t]
\centering
\caption{All metrics, HOSE. RMSE and MAE in units of $10^{-4}$. The own-history GBM is the best legal model at
every horizon; the $h1$ GBM+market 2.5126 is a thin-market spike artifact, not the best. Bold marks the QLIKE that
is significantly best by a date-clustered Diebold--Mariano test; unbolded values are not DM-distinguishable from
the best.}
\label{tab:hose-allmetrics}
\footnotesize
\setlength{\tabcolsep}{6pt}
\begin{tabular}{llcccc}
\toprule
$h$ & Model & QLIKE & RMSE & MAE & $R^2$ \\
\midrule
1  & HAR         & 1.8117 & 8.126 & 5.193 & 0.020 \\
1  & HARQ        & 1.7854 & 7.993 & 5.062 & 0.051 \\
1  & GBM         & \textbf{1.5679} & 7.348 & 3.944 & 0.198 \\
1  & GBM+earn    & 1.5730 & 7.401 & 3.966 & 0.187 \\
1  & GBM+market  & 2.5126 & 7.397 & 3.926 & 0.188 \\
1  & GBM+corr    & 1.5692 & 7.375 & 3.941 & 0.192 \\
1  & GBM+sector  & 1.5791 & 7.414 & 3.992 & 0.184 \\
1  & GBM+placebo & 1.5739 & 7.434 & 3.947 & 0.180 \\
\midrule
5  & HAR         & 1.8218 & 8.140 & 5.227 & 0.009 \\
5  & HARQ        & 1.8075 & 8.074 & 5.152 & 0.025 \\
5  & GBM         & \textbf{1.6478} & 7.613 & 4.265 & 0.133 \\
5  & GBM+earn    & 1.6485 & 7.613 & 4.276 & 0.133 \\
5  & GBM+market  & 1.6990 & 7.615 & 4.223 & 0.133 \\
5  & GBM+corr    & 1.6474 & 7.612 & 4.247 & 0.133 \\
5  & GBM+sector  & 1.6530 & 7.618 & 4.242 & 0.132 \\
5  & GBM+placebo & 1.6527 & 7.632 & 4.252 & 0.129 \\
\midrule
10 & HAR         & 1.8277 & 8.161 & 5.250 & 0.004 \\
10 & HARQ        & 1.8180 & 8.118 & 5.198 & 0.014 \\
10 & GBM         & \textbf{1.6842} & 7.724 & 4.425 & 0.107 \\
10 & GBM+earn    & 1.6862 & 7.721 & 4.441 & 0.108 \\
10 & GBM+market  & 1.6854 & 7.723 & 4.369 & 0.108 \\
10 & GBM+corr    & 1.6837 & 7.719 & 4.396 & 0.109 \\
10 & GBM+sector  & 1.6868 & 7.733 & 4.413 & 0.105 \\
10 & GBM+placebo & 1.6851 & 7.731 & 4.418 & 0.106 \\
\midrule
22 & HAR         & 1.8355 & 8.195 & 5.282 & $-0.002$ \\
22 & HARQ        & 1.8299 & 8.171 & 5.250 & 0.004 \\
22 & GBM         & \textbf{1.7263} & 7.881 & 4.615 & 0.073 \\
22 & GBM+earn    & 1.7262 & 7.881 & 4.627 & 0.073 \\
22 & GBM+market  & 1.7298 & 7.879 & 4.546 & 0.074 \\
22 & GBM+corr    & 1.7276 & 7.882 & 4.591 & 0.073 \\
22 & GBM+sector  & 1.7272 & 7.880 & 4.598 & 0.074 \\
22 & GBM+placebo & 1.7268 & 7.889 & 4.610 & 0.071 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Earnings does not transfer to Vietnam.} Using real crawled Vietnam announcement dates, GBM+earn ties or
...
\textbf{The oracle bound confirms the signal is contemporaneous.} Replacing the neighbor value at the origin with its
...
\textbf{Earnings coverage.} The Vietnam announcement dates combine the State Securities Commission (SSC) disclosure
...
\textbf{Takeaway.} The graph null replicates on a second, structurally different market. The earnings lever is
...
\bibitem{gnnhar} Zhang, C., Pu, X., Cucuringu, M., Dong, X.: Forecasting realized volatility with spillover effects: perspectives from graph neural networks. Int. J. Forecasting (2024). arXiv:2308.01419
\end{thebibliography}
