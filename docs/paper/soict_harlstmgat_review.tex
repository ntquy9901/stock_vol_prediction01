...
fact that stocks move together. The outcome is metric-dependent, so we report all five measures. On the liquid VN30 and VN100 indices the
simple HAR benchmark has the lowest QLIKE (the volatility-specific loss) at every horizon, with no learned
model significantly better on that loss; the deep and graph models, however, attain the lower mean-squared and
absolute error (MSE, RMSE, MAE) at the short horizons. Re-running the identical, un-retuned setup on the
broader Vietnamese market and on the U.S. S\&P 500 shows the split also depends on liquidity: on the thinly
traded HNX exchange the deep and graph models are more accurate than HAR across all five metrics at several
horizons, while on the liquid markets HAR keeps the lowest QLIKE but the learned models keep the lower squared
and absolute error. Adding the graph rarely improves on the deep model beyond training-seed noise. We report every result with explicit data-quality caveats rather than as a uniform win.
