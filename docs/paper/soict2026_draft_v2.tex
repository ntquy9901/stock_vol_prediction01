% SOICT 2026 submission draft (v2) --- Springer LNCS format.
% v2 (2026-08-05): reframed the primary baseline. The classical HAR econometric model (linear
% regression on the three HAR volatility scales) is now the primary external baseline; the
% price-only LSTM--GAT backbone (identical to the full model but without the news branch) moved to
% an ablation that isolates the news branch's marginal contribution. Classical HAR numbers come
% from the rerun of src/har_baseline/train.py after the per-ticker temporal-split fix
% (results/har_baseline_2026-08-05_224208/). v1 is the frozen prior version.
...
% Ablation paired t-tests across 3 seeds (price-only backbone vs gated news fusion).
...
input-independent gate, so each ticker admits a different amount of news signal. Against the
classical Heterogeneous Autoregressive (HAR) model, the standard econometric baseline for daily
volatility, the full news model lowers five-day-ahead test QLIKE from \qlikeHARc{} to \qlikeNews{}
and raises $R^2$ from \rsqHARc{} to \rsqNews{}, while the classical HAR stays lower on absolute
error (RMSE \rmseHARc{} vs.\ \rmseNews{}). To isolate what the news branch itself contributes, we
ablate it against an identical price-only backbone across three seeds at a matched budget: the news
branch lowers QLIKE from \qlikeBB{} to \qlikeNews{} (paired $t=\qliketstat$) and RMSE from \rmseBB{}
to \rmseNews{} (paired $t=\rmsetstat$), both passing a paired $t$-test. Directional accuracy does
not improve for any model (all near \diraccNews\%, ablation paired $t=\diracctstat$, below the 5\%
level). We show this is a property of the target rather than a model defect: the day-to-day change
in the single-day Parkinson volatility estimator is anti-persistent (sign lag-1 autocorrelation
$-0.30$, negative for all \ntick{} tickers), and model-free persistence and trailing-mean
forecasters reach the same $\approx 49\%$ ceiling while attaining negative $R^2$. News therefore
improves the forecast magnitude that the model can learn, and leaves untouched a direction task that
is near-random for every forecaster. We report the finding on a Vietnamese emerging market where
news-augmented volatility forecasting is largely unstudied.
...
\item \textbf{Direction is near-random by construction.} We show that no model improves directional
accuracy, and that the $\approx 48\%$ figure is the intrinsic ceiling of the task: the daily
Parkinson target's day-to-day change is anti-persistent, and model-free forecasters reach the same
ceiling (Section~\ref{sec:results} and Section~\ref{sec:discussion}).
\end{enumerate}

% Results preview.
On \ntick{} VN30 tickers over a temporal split, the gated news model attains test $R^2=\rsqNews$,
above both the classical HAR baseline (\rsqHARc) and the price-only backbone (\rsqBB), and the QLIKE
and RMSE gains from news hold in the same direction on all three seeds of the ablation. The
directional-accuracy result reframes a metric that earlier drafts of this project reported as high
(68--72\%) but which a measurement artifact had inflated; the corrected figure is near coin-flip,
and we explain it as a property of the data (Section~\ref{sec:discussion}).
...
we can defend, we repeat both the price-only backbone and the gated news model on three independent
seeds (42, 123, 2026) at this identical budget and report the mean, standard deviation, and a paired
$t$-test across seeds. We report results from the pipeline after two corrections described in
Section~\ref{sec:results}; earlier deep baselines in the project predate those corrections and are
not directly comparable, so we exclude them from the comparison.
...
result plainly and turn to a controlled ablation to measure what the news branch itself buys.

\subsection{Ablation: the news branch lowers QLIKE and RMSE across seeds}
\label{sec:ablation}
To isolate the news branch, we compare the full model against the price-only backbone that shares
every other component, over the same three seeds. Table~\ref{tab:ablation} reports the result.
Removing the news branch raises QLIKE from \qlikeNews{} to \qlikeBB{} and RMSE from \rmseNews{} to
\rmseBB{}; equivalently, adding it lowers both. A paired $t$-test across the three seeds gives
$t=\qliketstat$ for QLIKE and $t=\rmsetstat$ for RMSE, both past the two-sided 5\% threshold for two
degrees of freedom ($|t|>4.30$). The gains hold in the same direction on every seed: per-seed QLIKE
differences are $-0.0198$, $-0.0204$, and $-0.0118$, and per-seed RMSE differences are $-0.000183$,
$-0.000228$, and $-0.000159$. The news branch also raises $R^2$ from \rsqBB{} to \rsqNews{} and
lowers MAE from \maeBB{} to \maeNews{}.

\begin{table}[t]
\caption{Ablation of the news branch: five-day-ahead test metrics, mean$\pm$std over three seeds
...
for $R^2$ and DirAcc. The paired $t$ is across the three seeds; $|t|>4.30$ passes at the 5\% level
...
\smallskip
\noindent\textbf{Takeaway.} The news branch, holding the rest of the model fixed, improves the
metrics that score the forecast magnitude, and the improvement survives a paired test across seeds
rather than resting on one run. The gain is modest in absolute terms (about 4\% of QLIKE) but
consistent in sign.

\subsection{News does not improve direction}
Directional accuracy tells the opposite story. In the ablation both models sit near 48\%, below the
50\% no-skill line, and the price-only backbone is nominally higher (\diraccBB\% vs.\ \diraccNews\%).
The per-seed direction differences ($-0.53$, $-1.18$, $-0.42$ percentage points, all favouring the
backbone) give a paired $t$ of $\diracctstat$, which does not pass the 5\% threshold. News neither
helps nor reliably hurts direction, and the classical HAR baseline sits at the same near-random
level (\diraccHARc\%).
...
The per-ticker gate lowers aggregate QLIKE and RMSE in the ablation, yet its learned values do not
line up with independent measures of which stocks should need news. Four methods (two
feature-importance analyses, an ablation on per-stock QLIKE change, and the learned gate itself)
rank the stocks differently, and we have no unified account of the disagreement. The gate earns its
place as an architectural mechanism that improves the aggregate forecast; it does not yet serve as a
readout of per-stock news relevance. We report this as an open question rather than claim an
...
price branch and add the structure they omit, gaining on QLIKE and $R^2$.
...
more would strengthen the QLIKE and RMSE ablation result. Second, the classical HAR baseline uses a
...
universe rather than the live index. Fourth, the horizon observation of Section~\ref{sec:discussion}
...
Vietnamese news for each VN30 stock. Against the classical HAR econometric baseline the full model
lowers five-day-ahead QLIKE and raises $R^2$, and an ablation that removes only the news branch
shows the branch lowers QLIKE and RMSE with a paired $t$-test across seeds, though the linear HAR
stays lower on absolute error. The same model does not improve directional accuracy, and we show
that no forecaster does: the day-to-day direction of daily Parkinson volatility is anti-persistent
and near-random for structural reasons. News improves the volatility magnitude a model can learn,
and leaves untouched a direction task that the data itself makes unlearnable. For a Vietnamese
emerging market, this maps out both what news buys and what it cannot.
