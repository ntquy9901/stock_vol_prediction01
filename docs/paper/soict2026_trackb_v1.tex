...
%   classical      : docs/reports/classical_baselines_h5_2026-08-09_182129.json
% ----------------------------------------------------------------------
...
\newcommand{\GARCHtestQ}{1.7610} % GARCH QLIKE (raw 1.76100)
...
QLIKE improvement at any of four horizons under a Diebold--Mariano test (verdict B, graph null). HAR and
HARQ tie the deep models on the level metrics (test QLIKE \HARtestQ{} and \HARQtestQ, $R^2\approx0.767$),
while GARCH-family models are far worse (test QLIKE $1.76$--$1.87$, $R^2\approx0$). We present G1 as the
...
(GARCH covers a 32-ticker subset) was not feasible within the submission window, so the DM tests are the
...
and GARCH(1,1), GJR-GARCH, and EGARCH per ticker on close-to-close log returns. The GARCH family covers 32
of \ntick{} tickers (one ticker lacks raw OHLCV to form returns); all other baselines cover the full set.
...
far worse on every level metric (test QLIKE~\GARCHtestQ{}--$1.87$, $R^2$ from~\GARCHtestRsq{} to $-0.003$),
...
\ntest{} observations except the GARCH family ($14{,}292$, 32/33 tickers). Deep rows are \nseed-seed means;
...
GARCH(1,1)        & $1.7610$          & $0.004761$          & $0.0031$          & $48.67$ \\
GJR-GARCH         & $1.8243$          & $0.004766$          & $0.0007$          & $48.65$ \\
EGARCH            & $1.8738$          & $0.004775$          & $-0.0031$         & $48.83$ \\
\bottomrule
\end{tabular}
\end{table}
...
correlation $k$-NN. A targeted sweep of these levers is in progress.
\emph{[Placeholder, to be updated with the sweep outcome: as of this draft the beat-HAR sweep has not
returned a configuration that clears the HAR/HARQ QLIKE tie on the held-out test at a
Diebold--Mariano-significant level; the parsimony finding stands pending that result.]}
...
range-based quarticity proxy and the GARCH family covers 32 of \ntick{} tickers, so those rows are
approximations on a near-identical basis. Fifth, low directional accuracy is a structural property of the
