...
invariance; a fully clean Yang--Zhang comparison would use split/dividend-adjusted prices (here the overnight
return is winsorized at $\pm0.20$), and on a single-seed S\&P 500 check the same HAR-X lead holds under Parkinson while the LSTM+GAT is marginally lower under Yang--Zhang (0.549 vs 0.565), a single-seed difference within the seed dispersion that a full five-seed run would be needed to confirm.
