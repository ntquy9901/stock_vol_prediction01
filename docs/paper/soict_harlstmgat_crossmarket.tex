...
far worse on both exchanges, with a large negative $R^2$ (down to $-37.5$ on HOSE $h1$) and a significantly
higher QLIKE than HAR-X in every cell ($p<0.001$), consistent with a frozen-train forecast on illiquid series.
...
\begin{table}[t]
\centering
\caption{S\&P 500 (714{,}520 masked obs over 1{,}624 test dates at $h1$; 5-seed means). Lower is better for
MSE/RMSE/MAE/QLIKE, higher for $R^2$. $^{\dagger}$~marks metrics where LSTM+GAT improves over the no-graph
LSTM. MSE $\times10^{-7}$, RMSE $\times10^{-4}$, MAE $\times10^{-4}$.}
\label{tab:sp500}
\footnotesize
...
1  & GARCH    & 351.906 & 59.322 & 17.452 & 1.0066 & -37.6813 \\
1  & LSTM     & 6.298 & 7.936 & 2.375 & 0.3786 & 0.3077 \\
1  & LSTM+GAT & 6.289$^{\dagger}$ & 7.930$^{\dagger}$ & \textbf{2.319}$^{\dagger}$ & 0.3667$^{\dagger}$ & 0.3087$^{\dagger}$ \\
\midrule
5  & HAR-X    & 8.087 & 8.993 & 2.695 & \textbf{0.4171} & 0.1110 \\
5  & GARCH    & 187.597 & 43.313 & 15.516 & 0.9885 & -19.6207 \\
5  & LSTM     & 7.921 & 8.900 & 2.643 & 0.4540 & 0.1293 \\
5  & LSTM+GAT & \textbf{7.896}$^{\dagger}$ & \textbf{8.886}$^{\dagger}$ & \textbf{2.559}$^{\dagger}$ & 0.4722 & \textbf{0.1321}$^{\dagger}$ \\
\midrule
\multicolumn{7}{l}{\emph{Extended horizons}}\\
10 & HAR-X    & 8.977 & 9.475 & 2.847 & \textbf{0.4642} & 0.0152 \\
10 & GARCH    & 360.640 & 60.053 & 17.430 & 1.0046 & -38.5632 \\
10 & LSTM     & 8.604 & 9.276 & 2.883 & 0.4879 & 0.0561 \\
10 & LSTM+GAT & \textbf{8.502}$^{\dagger}$ & \textbf{9.220}$^{\dagger}$ & \textbf{2.748}$^{\dagger}$ & 0.4892 & \textbf{0.0674}$^{\dagger}$ \\
\midrule
22 & HAR-X    & 9.655 & 9.826 & 2.988 & \textbf{0.5848} & -0.0578 \\
22 & GARCH    & 336.895 & 58.043 & 17.600 & 0.9968 & -35.9096 \\
22 & LSTM     & 9.307 & 9.647 & 3.054 & 0.6840 & -0.0197 \\
22 & LSTM+GAT & \textbf{9.283}$^{\dagger}$ & \textbf{9.635}$^{\dagger}$ & \textbf{2.972}$^{\dagger}$ & 0.7018 & \textbf{-0.0170}$^{\dagger}$ \\
\bottomrule
\end{tabular}
\end{table}
...
far worse, with QLIKE near $1.0$ and a large negative $R^2$ at every horizon. The negative $R^2$ at $h22$ for
