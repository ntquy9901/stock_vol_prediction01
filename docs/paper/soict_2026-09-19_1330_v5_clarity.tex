...
leaf-graph smoothing---VolTree, with three contributions. First, the pooled endogenous XGBoost
...
S\&P~500, cutting QLIKE by a further 2.7 to 3.2\% at every horizon (Diebold-Mariano $p<0.001$); on 405
Vietnam HOSE tickers it does not transfer, localizing the gain to the S\&P~500. Third, a leaf-cooccurrence
...
\subsection{Datasets}
\textbf{Data sources.} Daily open-high-low-close-volume (OHLCV) data for the 497 S\&P~500 tickers is from Yahoo Finance; the 405 HOSE
...
with remaining settings following the published implementation. The XGBoost hyperparameters were
pre-specified (not tuned per fold); GNNHAR used validation-based early stopping under the same protocol.
All share the XGBoost model's target, protocol, and positivity floor. The XGB, XGB+E, and XGB+LG variants (Section~\ref{sec:voltree}) are ablations of VolTree, not
external baselines.
...
loss-differential series with a Newey--West heteroskedasticity- and autocorrelation-consistent (HAC) estimator~\cite{neweywest1987} at lag $h-1$ to accommodate
...
Table~\ref{tab:summary} summarises the headline metrics. Among the four external baselines in
Table~\ref{tab:summary} (GARCH, HAR, GNNHAR, VolTree), VolTree has the lowest QLIKE at every horizon in
...
The exact Diebold-Mariano $p$-values for the primary comparisons (S\&P~500 earnings increment; HOSE
leaf-graph at $h1$/$h5$) are below $0.001$; Table~\ref{tab:detail} markers use $p<0.05$ for compactness.
...
horizons, per-fold values span 0.0 to 0.8 (per-horizon medians about 0.40 at $h1$, 0.15 to 0.20 at longer
horizons), so the graph is materially active there. On the S\&P~500 sample the construction has no effect:
...
(ticker-days where the GARCH(1,1) fit converged to a non-degenerate, stationary solution; $\sim$99.5\% of
rows on the S\&P~500, $\sim$98.6\% on HOSE), a slightly different sample than the other models.
...
earnings windows), whereas on HOSE it stays flat.

\begin{figure}[t]
\centering
\includegraphics[width=0.56\linewidth]{fig_earnings_event_study.pdf}
\caption{Ex-post event study: mean and median realized Parkinson variance, normalised by each ticker's
median, around realized earnings dates (panel-specific $y$-scales; S\&P~500 $n{=}42{,}780$ events, HOSE
$n{=}5{,}256$). The S\&P~500 median rises to about 2.75 times baseline on the announcement day and returns
...
\textbf{Where the error concentrates.} The endogenous XGBoost model beats HAR across the calm deciles of realized variance (D0 the calmest, D9
the storm decile), yet its remaining QLIKE concentrates in D9, which alone carries 43\% of the model's total test
QLIKE (Fig.~\ref{fig:diagnostics}). It under-forecasts exactly there, its median forecast-to-realized
ratio falling below one while every calm decile sits above one. Permutation importance~\cite{breiman2001}
ranks the three HAR lags far above every momentum and auxiliary term, so the endogenous feature set does not
fully explain storm-regime variation. Because a release date can be estimated before the origin from the
firm's reporting cadence and drives the announcement-driven part of these spikes, a cadence-estimated
earnings feature is the natural lever for this residual.
...
well-covered 2025-2026 folds, so it does not rest on missing dates. On HOSE, by contrast, the leaf-cooccurrence graph is a small
but significant positive lever at the one- and five-day horizons (Section~\ref{sec:robust}).
...
prediction error of 32 days, against 2 days on the S\&P~500, yielding substantially noisier origin-time schedule estimates and the
likely reason the lever does not transfer; we therefore claim no incremental earnings value on HOSE under
the available real dates, not a definitive full-history null.
...
subset; the leaf-graph weight $\alpha$ is selected on a short 22-date validation slice; no explicit
...
configuration. The regime diagnostics (Fig.~\ref{fig:diagnostics}) use a single chronological split and
seed, so they are exploratory, and the GNNHAR comparison reflects the validation-selected variant.
...
\bibitem{breiman2001} Breiman, L.: Random forests. Machine Learning \textbf{45}(1), 5--32 (2001)
