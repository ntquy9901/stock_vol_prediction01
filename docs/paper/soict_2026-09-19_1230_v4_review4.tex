...
$p<0.001$); on 405 tickers from Vietnam's Ho Chi Minh Stock Exchange (HOSE) the earnings lever does not
...
gains remain significant after excluding selected market-stress windows. On the S\&P~500 sample, by
contrast, the weight collapses toward zero and the graph has no effect. The graph is therefore consistent with a
market-specific cross-sectional denoising effect rather than a universally beneficial graph mechanism.
VolTree is thus a unified model that stays numerically close to the best
market-specific variant; the two components provide market-specific rather than universally additive gains.
...
The standard linear approach is the HAR family~\cite{corsi2009}, which predicts
...
learned GNNHAR on QLIKE at every horizon. Second, a
...
adds both the earnings block and leaf-graph smoothing. Its earnings and leaf-graph components are optional
and their benefit is market-specific.
...
GNNHAR~\cite{gnnhar2024} is a learned graph neural network built on a per-fold correlation adjacency
estimated only from that fold's training window (frozen through validation and test, so no leakage) over the
three HAR lags: two graph-convolution layers (hidden width 9), trained with Adam (learning rate $10^{-3}$,
weight decay $10^{-5}$, gradient clipping at 1.0) for at least 20 and at most 120 epochs with early stopping
on validation QLIKE (patience 15), run as a three-seed ensemble; we report the validation-selected variant,
with remaining settings following the published implementation and not further retuned. The XGBoost
hyperparameters were pre-specified and were not tuned per fold; GNNHAR used validation-based early stopping
under the same chronological protocol. All
...
No earnings feature is generated until at least three historical releases are available, so the first three
per ticker are anchors and not exposed as features. This is a causal cadence-based expected schedule that is
conditionally safe for the evaluated horizons (only inter-release gaps strictly before each event enter; a
caveat quantified in the Limitations); using realized dates instead yields an optimistic upper bound
...
seed-averaged prediction; because the booster is effectively deterministic here, the two coincide up to
negligible seed variation. The weight $\alpha\in[0,1]$ is selected from a grid on the validation slice only,
...
MAE. We first average loss differentials across stocks within each date to remove contemporaneous
cross-sectional dependence; the Diebold-Mariano test~\cite{dm1995} is then applied to the daily
loss-differential series with a Newey--West HAC estimator~\cite{neweywest1987} at lag $h-1$ to accommodate
serial dependence in $h$-step-ahead forecasts, plus the Harvey-Leybourne-Newbold small-sample
correction~\cite{hln1997}. The primary pre-specified comparisons are XGB vs.\ HAR, XGB+E vs.\ XGB, XGB+LG
...
VolTree beats GARCH and HAR on QLIKE at every horizon, and the learned GNNHAR does not beat
...
XGB), and the leaf-graph is the only component that lowers QLIKE, so XGB+LG attains the lowest HOSE QLIKE at
every horizon (significantly at $h1$ and $h5$) and VolTree closely tracks it without providing an additional
improvement.
...
The weight $\alpha$ is selected per fold on validation and is not constant: across horizons the mean
validation-selected $\alpha$ ranged 0.22 to 0.35, with per-fold values spanning 0.0 to 0.8 (per-horizon
medians about 0.40 at $h1$ and 0.15 to 0.20 at longer horizons), so the graph is materially active there.
On the S\&P~500 sample the construction
...
HAR), an optimistic upper bound because the realized date is unknown at the forecast origin.
...
6.6\%, $h22$ 8.1\%, the larger horizon-growing share) and an incremental earnings component
...
days runs about 1.1 times baseline, against the pronounced S\&P~500 spike. One possible explanation, not
directly tested here, is that Vietnam's daily price limits delay or dampen announcement-day adjustment. The
result holds in the well-covered 2025-2026
...
under the no-subsampling configuration. The regime diagnostics (Fig.~\ref{fig:diagnostics}) use a single
...
realized-date upper bound. The analysis covers two markets only. The conclusions are limited to daily
OHLC-based Parkinson variance and may not transfer to intraday realized or option-implied volatility.
