...
\title{VolGA: A Temporal Graph-Attention Model for Multi-Horizon Stock Volatility Forecasting}
...
\begin{abstract}
Forecasting daily stock-price volatility --- how much a stock's price is likely to swing on a given day --- is a
core input to risk management and option pricing: a risk manager sizing exposure or a trader pricing an option
needs to know not only where a price may go but how turbulent the path will be.

We evaluate \emph{VolGA} (Volatility Graph-Attention), a model that fuses a per-stock temporal branch with a graph-attention branch, for multi-horizon volatility forecasting, using Vietnam's Hanoi Stock
...
market VolGA attains the lower forecast error, with a significantly lower QLIKE than HAR at three of the
four horizons and the lowest error on all four metrics at the daily horizon; the market and estimator ablations
report the corresponding results across the other markets and volatility targets.
...
This paper asks \emph{where} deep and graph models improve on the linear HAR benchmark, across several markets. We take the Hanoi exchange (HNX) as the primary test market and add the VN30 and
VN100 indices, the broader HOSE exchange and the U.S. S\&P 500 as a cross-market comparison, with GARCH(1,1) as a
classical reference. Holding the five inputs and the data design fixed, we compare an extended HAR (HAR-X), an
LSTM, and VolGA, all trained and evaluated on the same panel with the same five features so the only difference
is the model. Our headline finding: on HNX VolGA achieves a significantly lower QLIKE than HAR at three of the four horizons and, at the daily horizon,
the lowest error on all four metrics; on the larger panels (VN100, HOSE) the learned models significantly
...
Our contributions are: (i) a multi-horizon, multi-market benchmark that holds the feature set and
data design fixed across HAR-X, an LSTM, and VolGA, spanning HNX, the VN30, VN100 and HOSE panels and the developed-market S\&P 500, so any measured difference is attributable to a
single component; (ii) the finding that the learned models' advantage concentrates where the linear benchmark is
weakest --- on HNX VolGA has a significantly lower QLIKE than HAR at three of four horizons and the lowest
error on all four metrics at the daily horizon, while on the larger panels the learned models
significantly reduce the absolute error and stay competitive on QLIKE --- together with a reading
...
notably the high share of zero-range (no-intraday-move) days on HNX and the unverified split
...
three feature-based models (HAR-X, LSTM, VolGA) use the same five node features per ticker at $t$: the daily Parkinson variance, its 5-day
...
\textbf{Volatility estimators.} The primary target is the Parkinson variance; the ablation of
Sec.~\ref{sec:abl} also uses the Rogers--Satchell and windowed Yang--Zhang estimators. From the daily open $O$,
high $H$, low $L$ and close $C$ ($\ln$ is the natural logarithm), each is a daily variance estimator: Parkinson
uses only the high--low range, Rogers--Satchell is drift-independent, and Yang--Zhang is windowed over $n{=}20$
days.
\begin{align*}
...
$\ln(O_t/C_{t-1})$ and the open-to-close return $\ln(C_t/O_t)$, $\overline{\sigma^2_{\mathrm{RS}}}$ is the
rolling mean of the daily Rogers--Satchell term, and $k=0.34/(1.34+(n{+}1)/(n{-}1))$~\cite{yangzhang2000,rogers1991}.
...
evaluation (dropout, weight decay, gradient clipping, ReduceLROnPlateau, early stopping). \emph{VolGA}
...
estimated on training rows only; the branches are concatenated into an MLP head. Comparing it against the same-feature no-graph LSTM isolates the graph's contribution. \emph{GARCH}: a classical GARCH(1,1)
benchmark fit per ticker on the training variance series; it forecasts the variance directly and does not use
the node features.

\textbf{Masked panel.} Not every stock is listed on every date. Rather than keep only the dates on which all
stocks trade (which discards most of the data), we keep every trading date and mask out stocks not yet listed,
with a mask-aware loss that ignores the masked stocks, so every model is trained and evaluated on the same
panel.
...
VN30, ten on VN100). We report two targeted contrasts --- LSTM vs HAR-X and VolGA vs LSTM --- on QLIKE and
...
Top-5 weighted two-hop attention) are concatenated into an MLP head; removing the GAT branch gives the same-feature LSTM.}
\label{fig:arch}
\end{figure}

\section{Data}
The primary test market is \textbf{HNX} (Hanoi Stock Exchange, 154 screened tickers); the
\textbf{VN30} and \textbf{VN100} indices, the broader \textbf{HOSE} exchange and the U.S. \textbf{S\&P 500}
(440 tickers) serve as a cross-market comparison. All use the masked panel of Sec.~3 with the five features, a
...
\section{Experiments}
The main comparison is HAR-X, GARCH(1,1) and the proposed model VolGA (directed volume$\to$Parkinson weighted
...
\section{Results: the HNX market}\label{sec:results}
\begin{table}[t]
\centering
...
On HNX the proposed model VolGA is the most accurate model at the daily horizon ($h1$) --- the lowest MSE, RMSE,
MAE and QLIKE --- and it keeps the lowest QLIKE at $h10$ and $h22$; GARCH is far worse on every metric. By the
date-clustered DM test VolGA has a significantly lower QLIKE than HAR-X at three of the four horizons
...
The deep temporal--graph model's advantage thus falls on the HNX market, where the linear benchmark
is weakest.
...
\begin{table}[t]
\centering
\caption{Graph ablation on HNX: the no-graph LSTM vs the full VolGA (HAR-X for reference). MSE $\times10^{-7}$,
RMSE/MAE $\times10^{-4}$.}
\label{tab:abl-graph}
\footnotesize
\setlength{\tabcolsep}{5pt}
% AUTHORITATIVE: generated by scripts/paper/build_hnx_tables.py (do not hand-edit numbers)
\input{generated/tab_abl_graph_hnx.tex}
\end{table}

\subsection{Other markets: VN100, VN30, HOSE and the S\&P 500}
On the VN30, VN100, HOSE and S\&P 500 markets the linear HAR-X keeps the lowest QLIKE, consistent with its standing as a
hard-to-beat benchmark for large-cap markets, while VolGA reduces the absolute error: on VN100 it has a
significantly lower MAE than HAR-X at $h1$ and $h5$ (Table~\ref{tab:dm}), and its QLIKE is not significantly
different from HAR-X. On the developed-market S\&P 500 (Table~\ref{tab:abl-sp500}) HAR-X has the lowest QLIKE at
the short horizons while VolGA has the lower absolute error at $h1$ and $h5$ (Table~\ref{tab:dm}).
...
\setlength{\tabcolsep}{5pt}
% AUTHORITATIVE: generated by scripts/paper/build_hnx_tables.py (do not hand-edit numbers)
\input{generated/tab_abl_vn30.tex}
\end{table}

\begin{table}[t]
\centering
\caption{Market ablation --- HOSE (352 tickers). Same scaling as Table~\ref{tab:abl-vn100}.}
\label{tab:abl-hose}
\footnotesize
\setlength{\tabcolsep}{5pt}
% AUTHORITATIVE: generated by scripts/paper/build_hnx_tables.py (do not hand-edit numbers)
\input{generated/tab_abl_hose.tex}
\end{table}

\begin{table}[t]
\centering
\caption{Date-clustered DM contrasts of VolGA vs HAR-X across all markets (HNX first), on QLIKE and MAE
($p$-value; the favoured model in parentheses; bold when $p<0.05$; unadjusted).}
\label{tab:dm}
\footnotesize
\setlength{\tabcolsep}{5pt}
% AUTHORITATIVE: generated by scripts/paper/build_hnx_tables.py (do not hand-edit numbers)
\input{generated/tab_dm_voltg.tex}
\end{table}

On the VN30 and VN100 panels HAR-X has the lowest QLIKE at every horizon and no learned model is significantly lower
...
\setlength{\tabcolsep}{5pt}
% AUTHORITATIVE: generated by scripts/paper/build_hnx_tables.py (do not hand-edit numbers)
\input{generated/tab_abl_sp500.tex}
\end{table}

\subsection{Alternative volatility estimators}
To check that the results are not specific to the Parkinson target, we re-run the pipeline with the target
replaced by the two other published estimators defined in the Method --- the single-day Rogers--Satchell and the
windowed Yang--Zhang --- across all markets. QLIKE is scale-invariant, so it is comparable across targets;
Yang--Zhang is a smoothed twenty-day level and is far easier to forecast, so its absolute QLIKE is much lower and
is not comparable to the single-day measures (shown for completeness). Tables~\ref{tab:est-park}--\ref{tab:est-rs}
report all four metrics with the markets ordered HNX first. On HNX, VolGA has the lower QLIKE under the
single-day Parkinson and Rogers--Satchell targets at every reported horizon, while on the larger markets HAR-X
keeps the lower QLIKE under the single-day estimators. The Rogers--Satchell runs cover $h\in\{1,5,10\}$; the
$h22$ cell is in progress.

\begin{table}[t]
\centering
\caption{Parkinson target: HAR-X vs VolGA on all four metrics, all markets (HNX first).
MSE $\times10^{-7}$, RMSE/MAE $\times10^{-4}$.}
\label{tab:est-park}
\footnotesize
\setlength{\tabcolsep}{5pt}
% AUTHORITATIVE: generated by scripts/paper/build_hnx_tables.py (do not hand-edit numbers)
\input{generated/tab_est_parkinson.tex}
\end{table}

\begin{table}[t]
\centering
\caption{Windowed Yang--Zhang target (a smoothed twenty-day level; its QLIKE is not comparable to the single-day
measures). Same scaling as Table~\ref{tab:est-park}.}
\label{tab:est-yz}
\footnotesize
\setlength{\tabcolsep}{5pt}
% AUTHORITATIVE: generated by scripts/paper/build_hnx_tables.py (do not hand-edit numbers)
\input{generated/tab_est_yang_zhang.tex}
\end{table}

\begin{table}[t]
\centering
\caption{Rogers--Satchell target (single-day). Same scaling as Table~\ref{tab:est-park}.}
\label{tab:est-rs}
\footnotesize
\setlength{\tabcolsep}{5pt}
% AUTHORITATIVE: generated by scripts/paper/build_hnx_tables.py (do not hand-edit numbers)
\input{generated/tab_est_rogers_satchell.tex}
\end{table}

\FloatBarrier
\section{Discussion}\label{sec:disc}
On the primary HNX market the proposed model VolGA gives the lower forecast error (Table~\ref{tab:main-hnx}): at
...
$p=0.18$ at $h5$); at $h1$ its absolute-error gain over HAR-X is also significant ($p<0.001$). On HNX the graph branch contributes over the same-feature no-graph LSTM at the longer horizons
($h10$: $p=0.015$; $h22$: $p=0.031$).

\textbf{Why VolGA helps most on HNX.} HNX is a smaller market whose stocks trade less actively --- a large
share of its trading days show no intraday price change --- so its day-to-day volatility is more irregular and
harder to forecast: every model, HAR-X included, has a higher QLIKE on HNX than on the other markets. A simple
linear rule such as HAR predicts tomorrow's volatility as a smooth weighted average of its recent past; on the
heavily traded markets that assumption is already close to optimal, leaving little for a more complex model to
add. On the more irregular HNX series the non-linear temporal branch and the cross-stock graph can capture
structure the linear rule misses --- so the added machinery earns its keep where the simple benchmark is
weakest.

On the VN100 and VN30 panels HAR-X keeps the lowest QLIKE at every horizon, with no learned model
significantly lower on that loss --- consistent with the difficulty of improving on HAR on large-cap markets ---
...
five-seed ensemble forecast, the graph effect on the VN panels is best read as an ensemble-level regularity
rather than a robust per-seed improvement. At $h10$ and $h22$ HAR-X has the lower MSE, RMSE, MAE and QLIKE on the
VN panels. The ranking is metric- and horizon-dependent, which is why all four metrics are reported with
...
multiple-comparison correction; on the VN and S\&P 500 panels the QLIKE comparison is a null
result (no learned model is significantly better than HAR-X there), which a correction would only reinforce; the
significant results we emphasise --- the HNX QLIKE gains over HAR-X and the VN100 MAE gains --- are
reported with their exact $p$-values, and the graph-vs-LSTM contrasts are unadjusted exploratory findings. \textbf{Capacity.} The VolGA
adds a graph branch and its parameters to the no-graph LSTM, so this contrast measures the effect
of adding the implemented graph branch, not of graph structure at matched capacity.

\section{Conclusion}
Across several markets --- the HNX exchange, the VN30, VN100 and HOSE panels and the
developed-market S\&P 500 --- we benchmarked a linear HAR-X against an LSTM and VolGA under one
leakage-free, panel-correct protocol with the same features and the same date-clustered Diebold--Mariano
inference. The deep temporal--graph model is most valuable on the HNX market: it attains a
...
gain over the same-feature LSTM at the longer horizons. On the larger panels the learned models
...
strong linear benchmark for large-cap markets. The ranking is therefore metric- and market-dependent, which
...
their added complexity on the HNX market, where the linear benchmark is weakest.
