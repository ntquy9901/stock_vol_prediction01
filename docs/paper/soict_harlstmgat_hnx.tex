...
basic input to risk management, option pricing, and portfolio decisions. On liquid large-cap markets a simple
linear benchmark, the HAR model, is famously hard to beat, so the useful question is not whether a deep model
wins everywhere but \emph{where} the added complexity earns its keep. We show that the answer is the illiquid,
nonlinear end of the market. Taking Vietnam's thinly traded Hanoi Stock Exchange (HNX) --- whose volatility is
markedly less linearly persistent than that of blue chips --- as the primary test market, alongside the liquid
VN30 and VN100 indices, the broader HOSE exchange and the U.S. S\&P 500 as a liquidity comparison, we evaluate
the HAR benchmark against a deep sequence model (an LSTM) and a graph-augmented model (LSTM+GAT) that links each
stock to a few related peers. All models share the same five inputs and forecast a high--low range measure of
daily volatility (the Parkinson estimator, a variance) at one, five, ten, and twenty-two trading days ahead,
judged by five error measures --- MSE, RMSE, MAE, $R^2$ and the volatility-specific QLIKE --- with a
significance test adapted to the fact that stocks move together.
On the illiquid HNX market the LSTM+GAT achieves a significantly lower QLIKE than HAR at three of the four
horizons, and at the daily horizon it is the most accurate model on all five metrics, with both its QLIKE and
absolute-error gains statistically significant; at the longer horizons the graph branch adds a further
significant QLIKE gain over the same-feature LSTM. On the liquid VN100 index the learned models significantly
reduce the absolute error (MAE) at most horizons while matching HAR on QLIKE (no significant difference). HAR
remains the strongest QLIKE model on the smallest panel (VN30) and the developed-market S\&P 500 at the short
horizons, consistent with its standing as a hard-to-beat linear benchmark. The practical message is that a deep
temporal--graph model earns its added complexity precisely where the linear benchmark is weakest --- the
illiquid, nonlinear HNX market --- and elsewhere improves the error metrics while staying competitive on QLIKE.
We report every result with explicit data-quality caveats and release code and data for reproduction.
...
This paper asks \emph{where} deep and graph models beat the linear HAR benchmark, and answers it along a
liquidity gradient. We take the illiquid Hanoi exchange (HNX) as the primary test market --- its volatility is
less linearly persistent, so the linear benchmark has more room to be beaten --- and add the liquid VN30 and
...
the lowest error on all five metrics; on the larger liquid panels (VN100, HOSE) the learned models significantly
...
error on all five metrics at the daily horizon, while on the larger liquid panels the learned models
...
The forecast target so far is the Parkinson variance, which uses only the intraday high--low range, so a natural
question is whether the model comparison depends on this choice. We re-run the full pipeline (HAR-X, LSTM,
LSTM+GAT; five seeds; the same masked panel, universe and protocol) with the target replaced by two other
\emph{published} range-based estimators computed from the same OHLC data: the windowed Yang--Zhang
estimator~\cite{yangzhang2000} and the Rogers--Satchell estimator~\cite{rogers1991}. Both are standard formulas
from their original papers, not indicator-form approximations. QLIKE is scale-invariant, so it is comparable
across targets. One caveat frames the reading: the Yang--Zhang estimator is a variance averaged over a
rolling twenty-day window, i.e.\ a \emph{smoothed level} rather than a single-day variance, so it is far more
persistent and much easier to forecast (its QLIKE is one to two orders of magnitude below the single-day
measures). We therefore include it for completeness but read the meaningful proxy robustness from the two
single-day measures, Parkinson and Rogers--Satchell.

\begin{table}[t]
\centering
\caption{Volatility-proxy robustness at horizon $h{=}1$: five-seed-mean QLIKE (lower is better) of each model
when the target is the Parkinson variance, the windowed Yang--Zhang estimator, or the Rogers--Satchell
estimator, on the same universe and protocol. Bold marks the lowest QLIKE per row. On the liquid VN30 and VN100
the linear HAR-X is lowest under all three measures; on the illiquid HNX a learned model is lowest under the two
single-day measures (Parkinson, Rogers--Satchell). Yang--Zhang is a smoothed twenty-day level (see text) and its
absolute QLIKE is not comparable to the single-day measures.}
\label{tab:proxy}
\footnotesize
...
VN30  & Yang--Zhang     & \textbf{0.006} & 0.007 & 0.013 \\
VN30  & Rogers--Satchell& \textbf{0.969} & 1.028 & 1.003 \\
\midrule
VN100 & Parkinson       & \textbf{0.512} & 0.621 & 0.552 \\
VN100 & Yang--Zhang     & \textbf{0.006} & 0.007 & 0.008 \\
VN100 & Rogers--Satchell& \textbf{1.067} & 1.186 & 1.085 \\
\midrule
HNX   & Parkinson       & 1.872 & 1.821 & \textbf{1.816} \\
HNX   & Yang--Zhang     & \textbf{0.030} & 0.031 & 0.032 \\
HNX   & Rogers--Satchell& 3.852 & 3.667 & \textbf{3.663} \\
\midrule
HOSE  & Parkinson       & \textbf{1.244} & 1.310 & 1.355 \\
HOSE  & Yang--Zhang     & \textbf{0.013} & 0.017 & 0.017 \\
HOSE  & Rogers--Satchell& 2.286 & 2.327 & \textbf{2.255} \\
\midrule
SP500 & Parkinson       & \textbf{0.362} & 0.638 & 0.543 \\
SP500 & Yang--Zhang     & 0.032 & \textbf{0.012} & 0.062 \\
SP500 & Rogers--Satchell& \textbf{0.381} & 0.908 & 0.761 \\
\bottomrule
\end{tabular}
\end{table}

On the liquid VN30 and VN100 panels the linear HAR-X has the lowest QLIKE under all three volatility measures,
so the ``HAR-X is best on liquid large-caps'' finding is robust to the proxy. On the illiquid HNX a learned
model has the lowest QLIKE under the two single-day measures --- the LSTM+GAT under both Parkinson and
Rogers--Satchell --- and the gap is significant (date-clustered DM, $p<0.001$ for LSTM+GAT vs.\ HAR-X under both),
so the ``a deep model helps on the illiquid exchange'' finding is likewise robust across the single-day proxies.
The smoothed Yang--Zhang target behaves differently, as expected from its construction: HAR-X is lowest on four
of five panels at $h{=}1$ (the LSTM leads only on the S\&P 500), reflecting that a persistent twenty-day level is
easy for the linear model. Across the longer horizons the Yang--Zhang comparison (complete for
$h\in\{1,5,10,22\}$) keeps HAR-X lowest on the liquid Vietnamese panels and a learned model lowest on HNX and the
S\&P 500. We keep Parkinson as the primary target for its simplicity and split invariance, and note that a fully
clean overnight-return comparison requires split/dividend-adjusted prices: the S\&P 500 series are adjusted,
whereas the Vietnamese OHLCV is not, so its overnight return is winsorized. The Rogers--Satchell comparison is
reported here at the short (primary) horizons $h\in\{1,5\}$; the Garman--Klass estimator and the longer-horizon
Rogers--Satchell runs are in progress and will appear in a subsequent revision. This table uses QLIKE because it
is scale-invariant and therefore comparable across estimator targets, whereas the mean-squared and absolute
errors carry the units of each target and cannot be compared across them; within each target the error-metric
ranking follows the main results (the learned models attain the lower MSE/RMSE/MAE while HAR-X keeps the lower
QLIKE on the liquid panels).

\section{Discussion}\label{sec:disc}
On the liquid VN30 and VN100 panels HAR-X keeps the lowest QLIKE at every horizon, with no learned model
significantly lower on that loss --- consistent with the difficulty of beating HAR on liquid large-caps ---
while the learned models' advantage there is on the error metrics. At $h1$ the deep models have the lowest
...
longer horizons on the liquid panels, but on the illiquid HNX the graph adds a significant QLIKE gain over the
no-graph LSTM at $h10$ and $h22$. Because the per-seed QLIKE standard deviation of the learned models ($0.02$--$0.07$) is of the
...
result (no learned model significantly beats HAR-X there), which a correction would only reinforce; the
...
significantly lower QLIKE than HAR-X at three of the four horizons and the lowest error on all five metrics at
...
is why all five metrics are reported with equal weight and significance is assessed with the date-clustered DM
