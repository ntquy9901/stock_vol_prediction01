...
four horizons and the lowest error on all four metrics at the daily horizon; the market and proxy ablations
...
(VN100, VN30, S\&P 500) and vary the volatility proxy (Yang--Zhang, Rogers--Satchell).
...
\subsection{Volatility proxy: Yang--Zhang and Rogers--Satchell}
To check that the HNX finding is not specific to the Parkinson target, we re-run the pipeline on HNX with the
target replaced by two other published range-based estimators computed from the same OHLC data: the windowed
Yang--Zhang~\cite{yangzhang2000} estimator (Table~\ref{tab:abl-yz}) and the Rogers--Satchell~\cite{rogers1991}
estimator (Table~\ref{tab:abl-rs}); QLIKE is scale-invariant and so comparable across targets. The Yang--Zhang
estimator is a variance averaged over a rolling twenty-day window --- a smoothed level that is easier to
forecast --- so it is included for completeness, whereas Rogers--Satchell is a single-day measure like
Parkinson. Under the single-day Rogers--Satchell target the LSTM+GAT has the lower QLIKE on HNX at every
reported horizon; under the smoothed Yang--Zhang target a learned model has the lower QLIKE at the longer
horizons. The HNX result is therefore not specific to the Parkinson target. The Rogers--Satchell runs cover
$h\in\{1,5,10\}$; the Garman--Klass estimator and the $h22$ cell are in progress.

\begin{table}[t]
\centering
\caption{Proxy ablation on HNX --- windowed Yang--Zhang target (a smoothed twenty-day level; its QLIKE level is
not comparable to the single-day measures). MSE $\times10^{-7}$, RMSE/MAE $\times10^{-4}$.}
\label{tab:abl-yz}
\footnotesize
\setlength{\tabcolsep}{5pt}
% AUTHORITATIVE: generated by scripts/paper/build_hnx_tables.py (do not hand-edit numbers)
...
\begin{table}[t]
\centering
\caption{Proxy ablation on HNX --- Rogers--Satchell target (single-day). Same scaling as
Table~\ref{tab:abl-yz}.}
\label{tab:abl-rs}
\footnotesize
\setlength{\tabcolsep}{5pt}
% AUTHORITATIVE: generated by scripts/paper/build_hnx_tables.py (do not hand-edit numbers)
...
$h10$ and $h22$ HAR-X has the lower MSE, RMSE, MAE and QLIKE on the liquid panels. The ranking is metric- and
horizon-dependent, which is why all four metrics are reported with equal weight. The date-clustered DM test matters for these
