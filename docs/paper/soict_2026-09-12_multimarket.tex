...
HAR and HARQ on QLIKE at every horizon, and the learned GNNHAR does not beat it: GNNHAR is worse than
...
GARCH(1,1) benchmark (QLIKE only); it is omitted (\texttt{--}) on the S\&P~500 owing to an unstable
per-stock fit. Bold marks the best value in each metric column per horizon; the QLIKE bold additionally
