...
% STATUS 2026-09-06: complete draft, all 3 markets filled from results/edge_hmatched/edgehm_*.json; passed the
%   mechanical + semantic (red-team) style gates and section checklists. FOLLOW-UPS before submission:
%   (a) formalize Related Work \cite with a splncs04 .bib; (b) \input the tables/*.tex into the Results tables;
%   (c) add the architecture figure; (d) optionally populate qlike_robust columns via a re-run.
\documentclass{llncs}
\usepackage{graphicx}
...
% NOTE: in-text author-year references pending a .bib (splncs04); formalize \cite before submission.
\textbf{HAR-family econometric models.} The Heterogeneous AutoRegressive model (Corsi, 2009) forecasts realized
variance from daily, weekly, and monthly averages, and remains a strong baseline. Jump-aware extensions add a
jump component (HAR-CJ; Corsi, Pirino \& Reno, 2010) or scale the autoregressive coefficients by realized
quarticity (HARQ; Bollerslev, Patton \& Quaedvlieg, 2016), improving accuracy around jumps. These extensions
rest on intraday high-frequency estimators (bipower variation; Barndorff-Nielsen \& Shephard, 2004) and are
linear and univariate, so they ignore cross-sectional spillover.

\textbf{Deep sequence models.} LSTM and transformer forecasters capture non-linear temporal structure in a
single series, but applied per stock they also ignore the cross-section.

\textbf{Graph spillover models.} Graph neural networks wire stocks by a learned or correlation edge to model
volatility spillover across a panel. Their edge typically fixes the lead--lag independent of the forecast
...
floor, so no model gains from a looser floor. We test accuracy differences with a Diebold--Mariano test
