...
% PLACEHOLDER: fill from edgehm_sp500_clean_h*.json after the Colab run. Table \input{tables/sp500_clean_tables.tex}.
...
VN100 and VN30 [S\&P 500 pending], from the train-to-test QLIKE and R\textsuperscript{2} gaps that
...
days. The advantage does not extend to QLIKE, where the parsimonious HAR-X is strongest at the weekly and
longer horizons and, on the small VN30 panel, at the daily horizon as well. A short-lived spillover signal and
QLIKE's sensitivity to small-variance days together explain why the graph improves squared error without
improving QLIKE. [S\&P 500, the primary market, extends this comparison to a larger panel; results pending.]
...
\section{Conclusion}
VolGA pairs a per-stock LSTM with a graph-attention layer over a horizon-matched volume-to-volatility edge.
Across the VN100 and VN30 panels the graph lowers squared and absolute error at the daily horizon, while the
linear HAR-X remains the strongest QLIKE model at the longer horizons; every learned model generalizes without
over- or under-fitting. [The S\&P 500 primary-market results and the final cross-market synthesis are pending
the large-panel run.]
