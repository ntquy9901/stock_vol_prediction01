...
\node[edgebox, below=8mm of gat] (edge) {Horizon-matched edge\\$\mathrm{corr}(\mathrm{vol}_i(t),\sigma^2_j(t{+}h))$\\Bonferroni + Top-$K$};
...
102 after screening) and VN30 (33 tickers) are Vietnamese panels from the Ho Chi Minh exchange, screened the
same way; both carry more distinct tickers than their nominal index size because index membership changes over
...
VolGA versus the no-graph LSTM (the value of the graph) and VolGA versus HAR-X (deep versus linear).
