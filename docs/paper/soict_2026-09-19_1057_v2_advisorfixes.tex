...
extra information helps. The endogenous model beats the HAR linear benchmark, a per-stock GARCH, and a
learned graph neural network (GNNHAR) on QLIKE at every horizon. We then study one external signal
(earnings) and one cross-sectional denoising mechanism (the leaf graph) on two structurally different
markets. On the S\&P~500, a forward-looking scheduled-earnings feature is a modest but reliable lever,
...
weight, lowers HOSE QLIKE at the one- and five-day horizons by 0.11 to 0.22\% (Diebold-Mariano $p<0.001$,
robust to regime spikes), whereas on the liquid S\&P~500 the weight collapses toward zero and the graph has
no effect. The graph is therefore consistent with a market-specific cross-sectional denoising effect
rather than a universally beneficial graph mechanism.
...
model is a strong, transparent baseline for daily volatility that beats HAR, a per-stock GARCH, and the
learned GNNHAR on QLIKE at every horizon. Second, a forward-looking scheduled-earnings feature is a modest
but reliable external lever on the S\&P~500, cutting QLIKE by a further 2.7 to 3.2\% over the endogenous
model at every horizon (Diebold-Mariano $p<0.001$); replicated on 405 Vietnam HOSE tickers the earnings
lever does not transfer, which localizes the gain to the S\&P~500. Third, a leaf-cooccurrence graph that
denoises each trading day's cross-section of forecasts lowers HOSE QLIKE at the one- and five-day horizons
(Diebold-Mariano-significant and robust to regime spikes) yet self-disables on the liquid S\&P~500,
suggesting its benefit may be specific to thinner cross-sections rather than universal.
...
markets~\cite{defond2007}---a plausible basis for heterogeneous S\&P~500-versus-HOSE responses. Since a
release schedule is forward-looking, the distance to the nearest release is a natural predictor, but a
leakage-safe design must distinguish a point-in-time schedule (predicted at the forecast origin) from
realized dates known only ex post. We extend this literature by evaluating a leakage-controlled
...
five log-volatility momentum terms. An external earnings block adds $\mathbb{R}^4$ of forward-looking
scheduled-earnings features, chiefly the distance from the forecast target to the nearest release, and a
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.44\textwidth]{fig_architecture.pdf}
\caption{VolTree architecture. Endogenous features ($\mathbb{R}^8$) and a forward-looking
scheduled-earnings block ($\mathbb{R}^4$) feed a gamma-loss gradient-boosted tree ensemble (XGBoost);
...
HAR lags (hidden width 9), a three-seed ensemble with early stopping on validation QLIKE; we report the
validation-selected variant, with graph construction and remaining settings following the published
implementation and not further retuned. All share the XGBoost model's target, protocol, and positivity
floor. The XGB, XGB+E, and XGB+LG variants (Section~\ref{sec:voltree}) are ablations of VolTree, not
external baselines.
...
\end{align}
where $\mu_{22},\sigma_{22}$ are the trailing 22-day mean and standard deviation of $\ell_i$. These
capture the one-day jump (\emph{mr\_change}), the week/fortnight trend (\emph{mr\_slope5},
\emph{mr\_slope10}), deviation from the weekly average (\emph{mr\_dev5}), and a monthly z-score
(\emph{mr\_z22}), summarizing the stock's short-run acceleration and regime level.

\textbf{Scheduled-earnings features (4 features, used by XGB+E).} Each earnings feature is evaluated at the
target day $t+h$ from scheduled release dates known weeks ahead, so the block is forward-looking. For the
headline results the release date is a point-in-time estimate predicted at the forecast origin from the
firm's own past cadence: the next date is
...
\end{align}
Here $\mathrm{earn\_prox}$ is a symmetric triangular proximity peaking at 1 on the announcement day,
$\mathrm{earn\_soon}$ a sharp $\pm 3$-day flag, $\mathrm{earn\_pre}$ the five-day pre-announcement run-up,
and $\mathrm{earn\_post}$ a longer ten-day post-announcement decay (post-earnings drift outlasts the
run-up). Away from any release all four are zero, so XGB+E reduces to XGB on ordinary days and activates
only in the roughly two-week window around a release.
...
$\hat y_i^{\,\mathrm{LG}}=(1-\alpha)\,\hat y_i+\alpha\,\overline{\hat y}_{\mathcal{N}(i)}$, the mean over
its neighbours. The weight $\alpha\in[0,1]$ is selected per fold on validation only and applied once to the
test window, so the graph is causal, switches itself off ($\alpha=0$) when smoothing does not help, and
introduces no temporal leakage since it spans only the same day's tickers.
...
$g_h=\lfloor 1.6h\rfloor+5$ calendar days, with the last 22 training dates held out as validation. A
...
($\texttt{subsample}=\texttt{colsample\_bytree}=1.0$). Each model averages a three-seed ensemble (seeds 0,
1, 2); with no subsampling the seeds differ only in tie-breaking, so the ensemble mainly reduces numerical
nondeterminism rather than adding diversity.
...
errors are dependent. The primary pre-registered comparisons are XGB vs.\ HAR, XGB+E vs.\ XGB, XGB+LG
...
Table~\ref{tab:summary} summarises the headline metrics: VolTree posts the best QLIKE at every horizon in
both markets, and the best value on all four metrics everywhere except the S\&P~500 ten-day horizon, where
linear HAR edges it on the squared-error metrics. VolTree beats GARCH and HAR on QLIKE at every horizon,
and the learned GNNHAR does not beat it---GNNHAR is worse at every horizon and worse than even linear HAR
at $h1$ (QLIKE 0.375 against 0.370). Table~\ref{tab:detail} then isolates which component drives this.
...
minimum among these four models; the component variants are in Table~\ref{tab:detail}.}
\label{tab:summary}
\footnotesize
...
Table~\ref{tab:detail} isolates the four XGB-family variants (XGB, XGB+LG, XGB+E, VolTree). The endogenous
...
tickers, right; walk-forward, 3-seed; lower is better). XGB is endogenous-only, XGB+E adds earnings,
XGB+LG adds leaf-graph smoothing, VolTree adds both. MSE $\times10^{-6}$, RMSE/MAE $\times10^{-3}$,
QLIKE unscaled (as in Table~\ref{tab:summary}). Bold marks the per-horizon column minimum; for QLIKE,
$^{*}$ is a DM-significant improvement over the ablation base ($p<0.05$, Section~\ref{sec:robust}) and
$^{\dagger}$ a numerical minimum that is not significant. Tests use unrounded losses; baselines are in
...
leaf structure and motivating XGBoost as the forecasting engine. On the liquid S\&P~500 the construction
has no effect: the validation-fit weight collapses toward zero (mean $\alpha$ 0.00 to 0.10) and the QLIKE
change stays within 0.07\% and never significant, so the graph switches itself off where the cross-section
is already well estimated.
...
Figure~\ref{fig:eventstudy} shows the median variance rising to about 2.75 times baseline on the
announcement day, motivating the earnings windows, whereas on HOSE the median stays flat.

\begin{figure}[t]
\centering
\includegraphics[width=0.7\linewidth]{figures/fig_earnings_event_study.pdf}
...
every offset. This is why the earnings feature helps the S\&P~500 but not HOSE.}
...
storm-regime variation. Because a scheduled earnings release is known weeks ahead and drives the
announcement-driven part of these spikes, a forward-looking earnings feature is the natural lever for the
residual the endogenous model cannot reach.

\begin{figure}[t]
\centering
\includegraphics[width=0.66\textwidth]{fig_diagnostics_2x2.pdf}
...
\textbf{Earnings do not fix the storm tail.} The top decile carries roughly ten times the calm-day QLIKE
with a forecast-to-realized ratio near 0.4: the model compresses toward the conditional mean because the
storm magnitude is absent from the current origin-time feature set.
...
every horizon (QLIKE worse by 4.9 to 7.3\%, DM $p<10^{-26}$).
...
days runs about 1.1 times baseline, against the pronounced S\&P~500 spike---plausibly because Vietnam's
daily price limits dampen announcement-day adjustments. The result holds in the well-covered 2025-2026
folds, so it does not rest on missing dates.

\textbf{The leaf-graph helps the thin market.} On HOSE the leaf-cooccurrence graph is a small but
significant positive lever at the one- and five-day horizons (magnitudes and significance in
Section~\ref{sec:robust}), while on the liquid S\&P~500 the same construction self-disables.

\subsection{Practical Interpretation}
No external feature is universally useful for daily volatility forecasting: scheduled earnings help the
strong-announcement S\&P~500 but not HOSE (where price limits mute the response), while leaf-based
smoothing helps the thin HOSE cross-section but self-disables on the liquid S\&P~500. A strong endogenous
model plus a market-appropriate external signal is what matters.
...
prediction error of 32 days, against 2 days on the S\&P~500. This irregular, multi-event-type
schedule---not a date-accuracy defect---makes the feature much noisier on HOSE, the likely reason the lever
does not transfer; we therefore claim no incremental earnings value on HOSE under the available real dates,
not a definitive full-history null.

\noindent\textbf{Leaf-graph scope.} The graph is a small thin-market lever---cross-sectional denoising of
the model's own forecasts, not new outside information---whose gains, though significant, are a few tenths
of a percent and associational rather than causal.

\noindent\textbf{Evaluation caveats.} The regime diagnostics (Fig.~\ref{fig:diagnostics}) use a single
chronological split and seed, so they are exploratory; the GNNHAR comparison reflects the
validation-selected variant. We test many hypotheses across horizons and markets, so individual $p$-values
...
realized-date upper bound. The analysis covers two markets only and is conditional on the Parkinson
daily-OHLC regime, not intraday or option-implied signals.

\section{Conclusion}
We built VolTree, a pooled stock-day gamma-loss gradient-boosted tree ensemble (XGBoost) over endogenous
features, extended by a forward-looking scheduled-earnings block and a leaf-cooccurrence graph that smooths
each trading day's cross-section. The endogenous model beats the HAR and GARCH benchmarks and a learned
GNNHAR on QLIKE at every horizon. On the S\&P~500 the scheduled-earnings feature cuts QLIKE by a further
2.7 to 3.2\%; replicated on 405 Vietnam HOSE tickers the endogenous model again wins but the earnings lever
does not transfer, while the leaf-cooccurrence graph is a small but significant lever at the shorter
horizons and self-disables on the liquid S\&P~500. Scheduled earnings and leaf-based smoothing thus provide
distinct, market-specific benefits rather than universally additive gains. Future work should extend beyond
two markets and to intraday or option-implied signals.
