...
\title{VolTree: 
Earnings-Aware Gradient Boosting with Leaf-Graph Smoothing for Stock Volatility Forecasting}
...
\email{24C12031@student.hcmus.edu.vn,  \{lthoang,tdhoang\}@fit.hcmus.edu.vn}
}
...
The ablation follows a leave-one-out design from the full model, matching the rows of
Table~\ref{tab:detail}: \textbf{VolTree} uses both the earnings block and leaf-graph smoothing;
\textbf{VolTree w/o E} removes the earnings block (endogenous features and leaf-graph only);
\textbf{VolTree w/o LG} removes leaf-graph smoothing (endogenous features and earnings only); and
\textbf{VolTree w/o E and LG} is the endogenous-only model. We call the two single-component variants the
\emph{earnings variant} (VolTree w/o LG) and the \emph{leaf-graph variant} (VolTree w/o E). The ablation
quantifies each component's contribution, which is market-specific.

\begin{figure}[t]
\centering
\includegraphics[width=1\textwidth]{fig_architecture.pdf}
...
All baselines share the XGBoost model's target, protocol, and positivity floor. The three ablation variants (Section~\ref{sec:voltree}) are reduced forms of VolTree, not
external baselines.

\subsection{Feature Construction}
\textbf{Endogenous features (8 features, used by all variants).} Every endogenous feature is observable at the
...
\textbf{Cadence-estimated earnings features (4 features, used by the earnings-bearing variants).} Each earnings feature is evaluated
...
longer post-announcement decay. Away from any release all four are zero, so the earnings block is inactive on ordinary
...
one value per (market, horizon, fold), and applied once to the test window; the two leaf-graph variants (VolTree and VolTree w/o E) each fit
...
2025-01-01, 2025-07-01, and 2026-01-01, each spanning the following six months, except the last fold, which runs to the end of the available data. For fold start $F_k$ and
...
no-subsampling configuration; repeated seeds were used only as an implementation-stability
...
test~\cite{dm1995} is then applied to this daily loss-differential series. The primary pre-specified comparisons are the endogenous model vs.\ HAR, the earnings increment and the leaf-graph increment over the endogenous model, and VolTree vs.\ its single-component ablations; the rest are secondary.
...
single-component variants---the earnings variant on the S\&P~500 and the leaf-graph variant on HOSE---attain marginally lower QLIKE than the full model. VolTree also posts the best value on all four metrics
...
Table~\ref{tab:detail} isolates each component. The endogenous model (VolTree w/o E and LG) already beats HAR and
GARCH on QLIKE at every horizon. On the S\&P~500 the earnings block drives the further improvement---the
earnings variant carries the bolded QLIKE in every block---while leaf-graph smoothing self-disables. On HOSE the pattern
reverses: the earnings block is inert (the earnings variant is indistinguishable from the endogenous model) and the leaf-graph is the only
component that lowers QLIKE, so the leaf-graph variant attains the lowest HOSE QLIKE at every horizon (significantly at $h1$
and $h5$) with VolTree closely tracking it.

\begin{table}[!t]
\centering
\caption{Component ablation of VolTree on the S\&P~500 and HOSE, evaluated using
walk-forward validation and averaged over three seeds. VolTree denotes the full model with both earnings features (E) and leaf-graph smoothing (LG);
\emph{VolTree w/o E} retains LG but excludes earnings features; \emph{VolTree w/o LG} retains E but
excludes leaf-graph smoothing; and \emph{VolTree w/o E and LG} is the endogenous-only XGBoost model. MSE is reported in $10^{-6}$, RMSE and MAE in
$10^{-3}$, and QLIKE is unscaled; all metrics are lower-is-better. Bold values indicate the minimum in each
market--horizon column. For the QLIKE comparisons (Section~\ref{sec:robust}), $^{*}$ indicates a
statistically significant improvement ($p<0.05$), whereas $^{\dagger}$ indicates a numerical but
statistically non-significant improvement over the endogenous-only baseline. Each single-component variant
is compared with the endogenous-only baseline, while the full VolTree model is compared separately with the
E-only and LG-only variants.}
\label{tab:detail}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{ll cccc cccc}
...
1 & VolTree w/o E        & 0.557 & 0.746 & 0.209 & 0.3573 & 0.534 & \textbf{0.730} & \textbf{0.390} & \textbf{1.5640$^{*}$} \\
1 & VolTree w/o LG       & \textbf{0.554} & \textbf{0.744} & 0.208 & \textbf{0.3457$^{*}$} & 0.534 & 0.731 & 0.392 & 1.5675 \\
1 & VolTree w/o E and LG & 0.558 & 0.747 & 0.209 & 0.3573 & 0.534 & 0.731 & 0.392 & 1.5675 \\
\midrule
5 & VolTree              & 0.612 & \textbf{0.782} & \textbf{0.225} & 0.3995 & \textbf{0.578} & \textbf{0.760} & \textbf{0.423} & \textbf{1.6455} \\
5 & VolTree w/o E        & 0.613 & 0.783 & 0.226 & 0.4122 & \textbf{0.578} & \textbf{0.760} & \textbf{0.423} & \textbf{1.6455$^{*}$} \\
5 & VolTree w/o LG       & \textbf{0.611} & \textbf{0.782} & \textbf{0.225} & \textbf{0.3992$^{*}$} & \textbf{0.578} & \textbf{0.760} & 0.425 & 1.6474 \\
5 & VolTree w/o E and LG & 0.613 & 0.783 & 0.226 & 0.4121 & \textbf{0.578} & \textbf{0.760} & 0.425 & 1.6474 \\
\midrule
10 & VolTree              & 0.629 & \textbf{0.793} & \textbf{0.231} & 0.4184 & \textbf{0.594} & \textbf{0.771} & \textbf{0.438} & 1.6815 \\
10 & VolTree w/o E        & 0.631 & 0.794 & 0.232 & 0.4312 & \textbf{0.594} & \textbf{0.771} & \textbf{0.438} & \textbf{1.6814$^{\dagger}$} \\
10 & VolTree w/o LG       & \textbf{0.628} & \textbf{0.793} & \textbf{0.231} & \textbf{0.4182$^{*}$} & 0.595 & \textbf{0.771} & 0.439 & 1.6818 \\
10 & VolTree w/o E and LG & 0.631 & 0.794 & 0.232 & 0.4311 & 0.595 & \textbf{0.771} & 0.439 & 1.6816 \\
\midrule
22 & VolTree              & \textbf{0.621} & \textbf{0.788} & \textbf{0.234} & \textbf{0.4340} & \textbf{0.621} & \textbf{0.788} & \textbf{0.458} & 1.7252 \\
22 & VolTree w/o E        & 0.623 & 0.789 & 0.236 & 0.4459 & \textbf{0.621} & \textbf{0.788} & \textbf{0.458} & \textbf{1.7250$^{\dagger}$} \\
22 & VolTree w/o LG       & \textbf{0.621} & \textbf{0.788} & \textbf{0.234} & \textbf{0.4340$^{*}$} & \textbf{0.621} & \textbf{0.788} & 0.459 & 1.7253 \\
22 & VolTree w/o E and LG & 0.623 & 0.789 & 0.236 & 0.4459 & \textbf{0.621} & \textbf{0.788} & 0.459 & 1.7251 \\
\bottomrule
\end{tabular}
\end{table}
...
The lever is endogenous denoising, not earnings: the no-earnings ablation (the leaf-graph variant, VolTree w/o E in
Table~\ref{tab:detail}) gains 0.22\% at $h1$ and 0.12\% at $h5$ (both DM $p<0.001$) over
the endogenous model, matching the rounded VolTree values at $h1$ and $h5$ because the HOSE earnings block is effectively
inert.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_earnings_event_study.pdf}
...
\textbf{The earnings feature is the lever.} In Table~\ref{tab:detail}, the earnings variant posts the lowest S\&P~500
...
decomposing into an endogenous-nonlinearity component (the endogenous model over HAR: $h1$ 3.4\%, $h5$ 5.6\%, $h10$ 6.6\%,
$h22$ 8.1\%, the horizon-growing share) and an incremental earnings component (adding earnings to the endogenous model: $h1$ 3.2\%,
...
\textbf{The endogenous model wins on HOSE too.} On HOSE under the identical protocol, the endogenous XGBoost model is again the
...
\textbf{Earnings does not transfer to Vietnam.} Using the point-in-time expected schedule, the earnings variant is within
0.02\% of the endogenous model at every horizon. The HOSE earnings event study (Fig.~\ref{fig:eventstudy}) shows no
announcement-day spike, unlike the S\&P~500; one possible explanation, not directly tested here, is that
Vietnam's daily price limits may delay or dampen the announcement-day adjustment.
...
\noindent\textbf{Design and evaluation caveats.} Genuine point-in-time
