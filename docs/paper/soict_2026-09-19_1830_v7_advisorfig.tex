...
\begin{table}[!t]
\centering
\caption{Leave-one-out ablation of VolTree (S\&P~500 left; HOSE
right; walk-forward; seeds averaged as a stability check, not independent replications; all metrics
lower-is-better, aggregated as in Table~\ref{tab:summary}). VolTree is the full model;
\emph{VolTree w/o E} ($=$ XGB+LG in the text) removes the earnings block, \emph{VolTree w/o LG} ($=$ XGB+E)
removes leaf-graph smoothing, and \emph{VolTree w/o E and LG} ($=$ XGB) is the endogenous-only model.
MSE $\times10^{-6}$, RMSE/MAE $\times10^{-3}$, QLIKE unscaled (as in Table~\ref{tab:summary}). Bold marks
the best (minimum) value in each market--horizon column. For QLIKE significance ($^{*}$, $p<0.05$, Section~\ref{sec:robust}): \emph{VolTree w/o LG}
(earnings) and \emph{VolTree w/o E} (leaf-graph) are each tested against \emph{VolTree w/o E and LG}; VolTree is tested against \emph{VolTree w/o E} and \emph{VolTree w/o LG} separately. $^{\dagger}$
marks a numerical improvement over the endogenous base that is not statistically significant, and no marker is shown
...
\begin{figure}[t]
\centering
\includegraphics[width=0.80\linewidth]{fig_earnings_event_study.pdf}
