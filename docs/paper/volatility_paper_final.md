...
lowest mean at h5, h10, and h22 and the price-only LSTM backbone the lowest at h1. Diebold-Mariano tests
...
In additional experiments (Sections 6.5–6.9) we estimate the learned models by QLIKE rather than MSE and
add a fair augmented-HAR (HAR-X) linear baseline that receives the same five features as the price LSTM.
Against this same-feature linear baseline the learned model attains a significantly lower QLIKE only at
the one-day horizon, and a linear model gains nothing from the two extra features, which attributes the
one-day result to the LSTM nonlinearity rather than to the added features; at the longer horizons the HAR
family is as good or better. A market-regime split shows the learned model's edge is confined to the 90%
of calm days at the short-to-mid horizons and reverses on the turbulent decile, and a sub-period analysis
...
### 6.5 Additional experiments: estimation criterion, GAT depth, market regimes

The following experiments extend the ablation above along the axes highlighted by Zhang et al. (2023).
They use the same leakage-safe basis but estimate every learned model by minimising QLIKE (rather than
MSE), train in mini-batches, and use data through 2026-08-14; the numbers are therefore not directly
comparable to Tables 1–4 and are reported as a separate three-seed study. Diebold-Mariano tests use the
HLN small-sample correction with a HAC lag of $h-1$, seed-ensembled, on one shared positivity floor.

**Estimation criterion (QLIKE vs MSE).** Estimating the full model by QLIKE rather than MSE gives a
comparable held-out QLIKE at h1 and h5 and a lower value at h22 (0.6735 to 0.6657). Under QLIKE
estimation the full model attains a lower QLIKE than HAR at h1 ($-3.01$, $p=0.003$), the two are
indistinguishable at h5 ($p=0.225$), and HAR is lower at h10 ($+2.45$, $p=0.014$) and h22 ($+4.42$,
$p<0.001$). Across variants the lowest-QLIKE configuration is parsimonious: the price-only LSTM at h1
(0.4553; $-5.52$ vs HAR, $p<0.001$) and the price LSTM with the graph branch and no news at h5 (0.5430;
$-3.08$, $p=0.002$). In the leave-one-out ablation the news branch and the per-ticker gate do not lower
QLIKE at the short horizons on this run.

### 6.6 Fair linear baseline (HAR-X)

The price LSTM uses five node features while classical HAR uses three own-history features. To separate
the contribution of the two extra features (a market factor and a volume z-score) from the LSTM
nonlinearity, an augmented-HAR (HAR-X) linear model is fitted on all five features by ordinary least
squares, with the same train-only scaler and positivity floor.

| Horizon | HAR | HAR-X | price-only LSTM | HAR-X vs HAR | LSTM vs HAR-X |
|---|---|---|---|---|---|
| h1 | 0.4633 | 0.4627 | 0.4547 | $-0.30$, $p=0.76$ | $-3.95$, $p<0.001$ (LSTM) |
| h5 | 0.5503 | 0.5486 | 0.5445 | $-1.31$, $p=0.19$ | $-1.80$, $p=0.07$ |
| h10 | 0.5933 | 0.5965 | 0.5945 | $+2.82$, $p=0.003$ (HAR) | $-1.11$, $p=0.27$ |
| h22 | 0.6474 | 0.6501 | 0.6602 | $+5.37$, $p<0.001$ (HAR) | $+3.76$, $p<0.001$ (HAR-X) |

Adding the two features to a linear model does not lower QLIKE at the short horizons (HAR-X versus HAR is
not significant at h1 or h5) and raises it at h10 and h22. Against this same-feature linear baseline the
price LSTM attains a lower QLIKE only at h1 ($p<0.001$), which attributes the h1 result to the LSTM
nonlinearity rather than to the extra features; at h5 the difference is not significant, and at h10 and
h22 the HAR family is lower.

### 6.7 GAT depth

A single GAT layer (one hop) is compared with two stacked layers (two hops), both QLIKE-estimated. Two
hops attain a lower QLIKE than one hop at h1 ($-2.81$, $p=0.005$) and h10 ($-5.04$, $p<0.001$) and are
indistinguishable at h5 and h22; one hop is not lower at any horizon, and reducing to one hop removes the
significant h1 advantage over HAR. A mean-average-distance diagnostic on the two-hop model at h1 (0.3063
after the first hop, 0.1915 after the second) shows the second hop increases node-embedding similarity,
yet the added receptive field still lowers QLIKE. This differs from Zhang et al., who report one hop is
sufficient on Dow-Jones constituents.

### 6.8 Market regimes

Test observations are split by realised target volatility into a calm regime (lower 90%) and a turbulent
regime (upper 10%). On calm days the full model attains a lower QLIKE than HAR at h1, h5, and h10 (all
$p<0.001$); on turbulent days HAR attains a lower QLIKE at h5, h10, and h22 (all $p<0.001$). Turbulent-day
QLIKE is about three to four times the calm-day value, so the turbulent decile dominates the pooled
average and turns the pooled h5 and h10 comparisons into a tie or a HAR result even though the model
attains a lower QLIKE on the 90% of days that are calm. Both models under-predict the turbulent decile,
and HAR under-predicts it less.

### 6.9 Temporal stability

Splitting the held-out test window into four sequential blocks and running the QLIKE Diebold-Mariano test
of the full model against HAR within each block shows the full model's advantage is concentrated in the
middle sub-periods (significant in the second block at every horizon) while the most recent block favours
...
QLIKE relative to the full model at h1, h5, and h22.

Additional experiments that estimate the learned models by QLIKE and add a fair same-feature linear
baseline (HAR-X) sharpen this picture. The two extra node features add nothing to a linear model, and the
learned model's only significant advantage over the fair linear baseline is at the one-day horizon, where
it is attributable to the LSTM nonlinearity; a market-regime split places that edge on calm days at the
