...
\begin{table}[t]
\centering
\caption{QLIKE-loss ablation --- QLIKE by horizon on both panels: HAR-X, and the LSTM and VolGA
trained on the MSE loss (subscript MSE; the values of Tables~\ref{tab:vn100main}, \ref{tab:vn30main})
versus trained directly on the QLIKE loss (subscript QL). Five-seed ensemble; lower is better; bold
when the model's QLIKE is below HAR-X. The daily-horizon QLIKE-loss advantage over HAR-X is
significant on both panels (DM $p\le0.006$, in text).}
\label{tab:qlikeloss}
\footnotesize
...
\noindent\textbf{Scope.} This is a Vietnamese-market case study on two index panels; we do not claim
the results generalise beyond them.
