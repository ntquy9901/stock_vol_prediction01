% SOICT 2026 submission --- GAT ablation, v1 tiếng Việt năm-seed (2026-08-15) --- Springer LNCS format.
% Content = bản dịch tiếng Việt của bản thảo năm-seed:
%   docs/paper/volatility_paper_draft_5seed_vi.md (nguồn số liệu/bảng chuẩn).
% Mọi số liệu, bảng, và kết quả được sao chép nguyên văn từ nguồn năm-seed; chỉ nội dung văn bản được dịch.
...
Qua năm seed và bốn horizon, khác biệt giữa HAR, mô hình đầy đủ, và các biến thể ablation là nhỏ trên
...
và h22 còn minus\_graph có giá trị thấp nhất tại h1. Các kiểm định Diebold--Mariano trên dự báo
...
với độ biến động biên độ ngày mai của một mã đích --- mã hóa một quan hệ dự báo, nhân quả (dẫn-trễ) mà
...
năm seed cùng các kiểm định ý nghĩa trên dự báo seed-ensembled (Mục~\ref{sec:setup}
và~\ref{sec:results}).
\end{enumerate}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{diagrams/volatility_architecture.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{diagrams/volatility_ablation.pdf}
...
hình được huấn luyện dưới năm seed (42, 123, 2026, 7, 2024); các metric test được báo cáo là mean(std)
qua năm seed, và các kiểm định Diebold--Mariano chạy trên các dự báo test seed-ensembled.
...
trên các dự báo seed-ensembled (dự báo trung bình theo từng quan sát qua các seed 42, 123, 2026, 7, 2024).
...
\label{sec:metrics}
Bảng~\ref{tab:metrics} báo cáo cả năm metric qua bốn horizon, mean(std) qua năm seed. Đọc qua các
horizon: trên các metric sai số bình phương (MSE, RMSE, $R^2$), các giá trị trung bình của các cấu hình
nằm trong khoảng 1\% so với nhau và hàng tốt nhất luân phiên (minus\_gate tại h1, FULL/LSTM-only tại h5,
LSTM-only tại h10, HAR tại h22); trên MAE khác biệt cũng nhỏ tương tự và giá trị trung bình thấp nhất
thay đổi theo horizon (HAR/minus\_graph tại h1, ba cấu hình gần bằng nhau quanh 5.98 tại h5, FULL tại h10,
minus\_graph tại h22); trên QLIKE, HAR có giá trị trung bình thấp nhất tại h5, h10, và h22 còn minus\_graph
có giá trị trung bình thấp nhất tại h1. Các độ lệch chuẩn báo cáo là nhỏ so với khoảng cách giữa các cấu
hình ngoại trừ tại h10, nơi các giá trị trung bình QLIKE của FULL và minus\_news mang độ phân tán seed
lớn hơn (std 0.028 và 0.044).

\begin{table}[t]
\caption{Các metric TEST held-out theo horizon, mean(std) qua năm seed (42, 123, 2026, 7, 2024). Thấp
...
minus\_graph & 4.03 (0.05) & 2.009 (0.012) & \textbf{5.41 (0.04)} & 0.8201 (0.0021) & \textbf{0.4798 (0.0036)} \\
minus\_gate  & \textbf{4.00 (0.08)} & \textbf{2.000 (0.021)} & 5.43 (0.06) & \textbf{0.8217 (0.0037)} & 0.4816 (0.0062) \\
minus\_news  & 4.02 (0.07) & 2.004 (0.017) & 5.44 (0.08) & 0.8209 (0.0030) & 0.4810 (0.0061) \\
...
minus\_graph & 5.21 (0.04) & 2.282 (0.010) & \textbf{5.98 (0.07)} & 0.7683 (0.0020) & 0.5795 (0.0054) \\
minus\_gate  & 5.21 (0.07) & 2.283 (0.016) & 5.99 (0.05) & 0.7680 (0.0032) & 0.5768 (0.0031) \\
minus\_news  & 5.22 (0.07) & 2.284 (0.015) & 5.99 (0.06) & 0.7679 (0.0031) & 0.5773 (0.0032) \\
...
minus\_graph & 5.57 (0.10) & 2.359 (0.022) & 6.31 (0.05) & 0.7530 (0.0046) & 0.6196 (0.0033) \\
minus\_gate  & 5.57 (0.10) & 2.360 (0.021) & 6.30 (0.09) & 0.7529 (0.0043) & 0.6242 (0.0040) \\
minus\_news  & 5.61 (0.12) & 2.369 (0.025) & 6.28 (0.11) & 0.7509 (0.0053) & 0.6464 (0.0443) \\
...
minus\_graph & 6.17 (0.12) & 2.485 (0.023) & \textbf{6.51 (0.04)} & 0.7234 (0.0052) & 0.6979 (0.0112) \\
minus\_gate  & 6.21 (0.19) & 2.493 (0.037) & 6.57 (0.09) & 0.7216 (0.0084) & 0.7082 (0.0113) \\
minus\_news  & 6.17 (0.11) & 2.484 (0.023) & 6.56 (0.07) & 0.7235 (0.0051) & 0.7078 (0.0091) \\
...
h1, h10, và h22 (gỡ nhánh GAT làm giảm QLIKE ở đó) và âm nhẹ tại h5 ($-0.0021$). \texttt{effect(gate)}
dương tại h1, h5, và h10 và âm nhẹ tại h22 ($-0.0008$). \texttt{effect(news)} dương tại h1 và h5 và âm
tại h10 ($-0.0095$) và h22 ($-0.0004$), nên gỡ nhánh tin tức làm tăng QLIKE chỉ tại các horizon dài hơn.
...
$\text{effect}(X)=\text{QLIKE}(\text{FULL})-\text{QLIKE}(\text{FULL}{-}X)$, trung bình năm seed. Âm $=$ gỡ
...
\label{sec:fullvshar}
Bảng~\ref{tab:fullvshar} ghép FULL và HAR trên bốn horizon. Trên RMSE và $R^2$ mô hình đầy đủ có giá trị
trung bình nhỉnh hơn tại h1 và h5 còn HAR nhỉnh hơn tại h10 và h22, nhưng các khác biệt nằm trong độ phân
tán seed (kiểm định DM sai số bình phương không có khác biệt có ý nghĩa tại bất kỳ horizon nào; MSE và
MAE theo cùng khuôn mẫu trong Bảng~\ref{tab:metrics}). Trên QLIKE, HAR có giá trị trung bình thấp hơn tại
cả bốn horizon, với khoảng cách mở rộng từ h1 (0.0018) đến h22 (0.0332). Bảng Diebold--Mariano
(Bảng~\ref{tab:dm}) kiểm tra các khoảng cách này qua các họ mất mát trên dự báo seed-ensembled: trên
QLIKE, mô hình đầy đủ không có khác biệt có ý nghĩa so với HAR tại h1, h5, và h10 và HAR thấp hơn có ý
nghĩa tại h22; trên sai số bình phương không có khác biệt có ý nghĩa tại bất kỳ horizon nào; trên sai số
tuyệt đối, mô hình đầy đủ thấp hơn có ý nghĩa tại h5.

\begin{table}[t]
\caption{FULL so với HAR, test held-out, cả bốn horizon, mean(std) qua năm seed. Các cột ghép mỗi metric
...
chỉ tại h1 và trên mất mát sai số bình phương chỉ tại h10; gỡ cổng theo từng mã có ý nghĩa trên QLIKE tại
h1 và h5; gỡ nhánh tin tức có ý nghĩa trên QLIKE tại h1 và h22. Backbone LSTM chỉ-giá có mất mát thấp hơn
hoặc bằng so với mô hình đầy đủ trên QLIKE tại h1. Qua cả ba họ mất mát, không cấu hình nào được ưu tiên
đồng nhất so với HAR.

\begin{table}[t]
\caption{Diebold--Mariano (HLN) trên test held-out, seed-ensembled (42, 123, 2026, 7, 2024), theo từng
horizon và từng họ mất mát. Mỗi ô là thống kê DM hiệu chỉnh HLN kèm $p$-value hai phía; in đậm đánh dấu
$p<0.05$. Một thống kê âm nghĩa là FULL có mất mát thấp hơn; một thống kê dương nghĩa là đối tượng so
sánh có mất mát thấp hơn. DM chạy trên ba họ mất mát theo từng quan sát: QLIKE, sai số bình phương (SE;
họ MSE/RMSE/$R^2$), và sai số tuyệt đối (AE; MAE). Độ trễ cắt HAC $h{-}1$; $n$ theo từng horizon là 14.596
...
FULL vs minus\_graph & $\mathbf{+6.16}$ ($\mathbf{p<0.001}$) & $-1.77$ ($p = 0.08$) & $\mathbf{+3.33}$ ($\mathbf{p<0.001}$) \\
FULL vs minus\_gate  & $\mathbf{+7.55}$ ($\mathbf{p<0.001}$) & $+1.57$ ($p = 0.12$) & $\mathbf{+5.52}$ ($\mathbf{p<0.001}$) \\
FULL vs minus\_news  & $\mathbf{+8.99}$ ($\mathbf{p<0.001}$) & $+0.74$ ($p = 0.46$) & $\mathbf{+4.81}$ ($\mathbf{p<0.001}$) \\
FULL vs LSTM-only   & $\mathbf{+8.77}$ ($\mathbf{p<0.001}$) & $\mathbf{-2.99}$ ($\mathbf{p<0.001}$) & $\mathbf{+2.68}$ ($\mathbf{p = 0.01}$) \\
\midrule
\multicolumn{4}{l}{\textit{h = 5 ngày giao dịch}}\\
FULL vs HAR         & $-1.11$ ($p = 0.27$) & $-0.99$ ($p = 0.32$) & $\mathbf{-3.97}$ ($\mathbf{p<0.001}$) \\
FULL vs minus\_graph & $-0.51$ ($p = 0.61$) & $-0.43$ ($p = 0.67$) & $-0.82$ ($p = 0.41$) \\
FULL vs minus\_gate  & $\mathbf{-5.27}$ ($\mathbf{p<0.001}$) & $-1.86$ ($p = 0.06$) & $\mathbf{-3.36}$ ($\mathbf{p<0.001}$) \\
FULL vs minus\_news  & $-0.03$ ($p = 0.97$) & $\mathbf{-2.42}$ ($\mathbf{p = 0.02}$) & $\mathbf{-2.54}$ ($\mathbf{p = 0.01}$) \\
...
FULL vs minus\_graph & $+1.55$ ($p = 0.12$) & $\mathbf{+2.09}$ ($\mathbf{p = 0.04}$) & $\mathbf{-4.53}$ ($\mathbf{p<0.001}$) \\
FULL vs minus\_gate  & $+0.45$ ($p = 0.65$) & $\mathbf{+2.09}$ ($\mathbf{p = 0.04}$) & $\mathbf{-4.18}$ ($\mathbf{p<0.001}$) \\
FULL vs minus\_news  & $+1.27$ ($p = 0.20$) & $+0.63$ ($p = 0.53$) & $-1.09$ ($p = 0.28$) \\
FULL vs LSTM-only   & $+1.78$ ($p = 0.08$) & $\mathbf{+2.35}$ ($\mathbf{p = 0.02}$) & $\mathbf{-3.74}$ ($\mathbf{p<0.001}$) \\
\midrule
\multicolumn{4}{l}{\textit{h = 22 ngày giao dịch}}\\
FULL vs HAR         & $\mathbf{+4.13}$ ($\mathbf{p<0.001}$) & $+1.73$ ($p = 0.08$) & $-0.41$ ($p = 0.68$) \\
FULL vs minus\_graph & $+1.40$ ($p = 0.16$) & $+0.84$ ($p = 0.40$) & $\mathbf{+2.80}$ ($\mathbf{p = 0.01}$) \\
FULL vs minus\_gate  & $-0.25$ ($p = 0.80$) & $+0.20$ ($p = 0.84$) & $+0.40$ ($p = 0.69$) \\
FULL vs minus\_news  & $\mathbf{-2.14}$ ($\mathbf{p = 0.03}$) & $+0.74$ ($p = 0.46$) & $+0.25$ ($p = 0.81$) \\
...
h1 ($p<0.001$) và không có khác biệt có ý nghĩa tại h5, h10, hay h22, nên cạnh dẫn-trễ có hướng --- thiết
...
và làm tăng nó có ý nghĩa tại h22 ($p=0.03$), nên nhánh tin tức làm giảm QLIKE chỉ tại h22. Với cổng, gỡ
nó làm giảm QLIKE có ý nghĩa tại h1 và làm tăng nó có ý nghĩa tại h5, và không có khác biệt có ý nghĩa
tại h10 hay h22. So với HAR, mô hình đầy đủ không có khác biệt QLIKE có ý nghĩa tại h1, h5, và h10 và có
QLIKE cao hơn có ý nghĩa tại h22 (Bảng~\ref{tab:dm}).
...
test năm seed 0.4803 / 0.5812 / 0.6195 / 0.6982 tại h1 / h5 / h10 / h22, và kiểm định DM seed-ensembled
phát hiện nó có QLIKE thấp hơn có ý nghĩa so với mô hình đầy đủ chỉ tại h1 (không có khác biệt có ý nghĩa
tại h5, h10, hay h22). So với HAR, cả LSTM-only và mô hình đầy đủ đều không có lợi thế QLIKE có ý nghĩa
tại h1--h10 và có QLIKE cao hơn HAR tại h22, nên mô hình thời gian sâu bằng HAR ở các horizon ngắn và
không vượt qua baseline tuyến tính ở horizon dài.

\subsubsection{Đọc tổng thể.}
Qua năm seed và cả năm metric, không cấu hình nào vượt HAR một cách nhất quán hoặc có ý nghĩa. Trên MSE,
...
trị thấp hơn có ý nghĩa tại h22, và backbone LSTM chỉ-giá có QLIKE thấp hơn có ý nghĩa so với mô hình đầy
đủ tại h1. Các thành phần đồ thị, cổng, và tin tức không cung cấp một cải thiện có ý nghĩa nhất quán qua
các horizon trên bất kỳ metric nào.
...
hàng ngày này không có. Thứ ba, các con số báo cáo tổng hợp năm seed (42, 123, 2026, 7, 2024) với các
kiểm định Diebold--Mariano seed-ensembled; một Model Confidence Set là công việc tương lai khả thi để
...
giao dịch qua năm seed. Qua cả năm metric, không cấu hình nào vượt HAR một cách nhất quán hoặc có ý nghĩa:
...
horizon ngắn --- gỡ nhánh GAT làm giảm QLIKE có ý nghĩa tại h1, gỡ cổng theo từng mã làm giảm QLIKE tại
h1 và làm tăng nó tại h5, và gỡ nhánh tin tức làm giảm QLIKE tại h1 trong khi làm tăng nó tại h22 --- và
một backbone LSTM chỉ-giá có QLIKE thấp hơn có ý nghĩa so với mô hình đầy đủ tại h1. Với một thị trường
mới nổi Việt Nam, nghiên cứu cho thấy cách xây dựng và đánh giá một cách trung thực một bộ dự báo tin tức
