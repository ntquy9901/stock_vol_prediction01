...
test~\cite{dm1995} is then applied to the daily
...
firm's reporting cadence and is associated with part of the announcement-related variation, a
cadence-estimated earnings feature is the natural lever for this residual.

\begin{figure}[t]
\centering
\includegraphics[width=0.52\textwidth]{fig_diagnostics_2x2.pdf}
...
ranking the three HAR lags far above every other feature. Panel (c) reports the per-decile median (not the
mean) forecast-to-realized ratio to reduce sensitivity to near-zero realized variances, and no winsorization
is applied. Single-split, single-seed exploratory diagnostic; headline metrics use the full walk-forward.}
...
within the 2022-onward test period. The regime
diagnostics (Fig.~\ref{fig:diagnostics}) use a single chronological split and
...
2.7 to 3.2\%; replicated on 405 HOSE-listed Vietnamese stocks the endogenous model again wins, but the earnings lever
