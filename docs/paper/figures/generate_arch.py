...
Shows the per-stock-per-day feature vector x_{i,t} = endogenous R^8 (3 HAR lags + 5 log-vol momentum)
plus an earnings block R^4, fed to a gamma-loss gradient-boosted tree ensemble (XGBoost) that
emits the per-stock multi-horizon Parkinson-variance forecast y_hat_{i,t+h}. A post-prediction
stage then smooths those forecasts across each test day's cross-section over a
leaf-cooccurrence kNN graph built from the XGBoost trees' leaf indices; this is prediction smoothing,
not a GNN and not an input feature. All stages are mandatory components of the model.

Layout: a VERTICAL top-to-bottom pipeline (OHLCV / earnings-calendar inputs at top -> feature-vector
boxes -> XGBoost -> per-stock forecast) with a panel at the bottom holding the
...
def feature_rows():
    """Feature-vector rows shown in the stack, bottom-to-top: 4 external (earnings) then 8
    endogenous. The cross-firm graph is NOT a feature row (the graph is a post-prediction step)."""
...
def main():
    ...
    rows = feature_rows()                              # earnings (reversed) then endogenous (reversed)
    ...
    box(ax, 5.5, 10.62, 3.0, 1.06,
        "scheduled earnings\ncalendar (external)", "#fdf3e3", 7.8, ec="#b8791f")
    ...
    box(ax, 5.3, 8.45, 3.4, 1.65,
        r"earnings  $\mathbb{R}^{4}$" + "\n" + _vjoin(ext), "#fdf3e3", 7.4, ec="#b8791f")
    arr(ax, 3.1, 10.55, 3.0, 10.12)                    # OHLCV -> endogenous block
    ...
    arr(ax, 3.0, 8.45, 4.35, 8.08, r"$x_{i,t}$", dy=0.20)   # endogenous -> XGBoost
    ...
    ax.text(tx, py + 1.40, r"leaf-graph smoothing ($\alpha\in[0,1]$ fit on validation)",
