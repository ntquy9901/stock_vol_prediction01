...
def main():
    ...
    brace(vy, vy + 4 * ch, r"$\mathbb{R}^{4}$ external (E)", "#b8791f")
    brace(vy + 4 * ch, vy + 12 * ch, r"$\mathbb{R}^{8}$ own-history", "#2f7d4a")
    ...
    # ---- top summary line ----
    ax.text(8.0, 8.5,
            r"GBM: $x\in\mathbb{R}^{8}$ (own)   $\cdot$   GBME: $\mathbb{R}^{12}$ (own$+$E)",
