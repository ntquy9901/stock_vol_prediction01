...
\author{Thanh-Quy Nguyen\inst{1} \and Trung-Hoang Le\inst{1} \and Duy-Hoang Tran\inst{1}}
...
ensemble (XGBoost) over own-history features, and we ask where extra information actually helps. The
own-history model beats the HAR linear benchmark, a per-stock GARCH, and a learned graph
...
universal one. All comparisons use an identical leakage-controlled walk-forward protocol scored on
QLIKE with date-clustered Diebold-Mariano tests.
...
We forecast daily Parkinson variance with a per-stock gamma-loss XGBoost model over own-history
...
does not beat the own-history model on either market.

This paper contributes three results. First, a per-stock gamma-loss XGBoost model over own-history
...
tree under a loss-guided grow policy, L2 penalty 1.0, minimum child weight 20, 3-seed ensemble). We
use XGBoost, rather than the scikit-learn HistGradientBoostingRegressor~\cite{pedregosa2011}, because
it exposes the per-tree leaf assignment used by the leaf-cooccurrence graph below; at matched capacity
the two engines agree on QLIKE to within about 0.2\% at every horizon on both markets, so the choice
does not change the own-history result. The own-history feature vector is $\mathbb{R}^8$: three HAR
lags~\cite{corsi2009} and five log-volatility momentum terms. An optional External earnings block
...
graph (denoted \textbf{XGB+E+LG}) optionally smooths the resulting forecasts across each day's
cross-section. Figure~\ref{fig:arch} shows the architecture.

\textbf{Baselines.} HAR~\cite{corsi2009} fits three own-history lags by ordinary least squares. GARCH is a per-stock GARCH(1,1) on daily
...
\subsection*{Features}
\textbf{Own-history block (8 features, used by XGB).} Every own-history feature is observable at the
...
(XGBoost) maps the feature vector to the multi-horizon Parkinson-variance forecast, which an optional
per-day leaf-cooccurrence graph smooths across the cross-section.}
\label{fig:arch}
\end{figure}

\section{Results: The Own-History Model Beats the Baselines}\label{sec:graph}
Table~\ref{tab:sp500} reports all four metrics for the S\&P~500 across GARCH, HAR, the learned
GNNHAR, and the XGBoost variants at every horizon: XGB (own-history only), XGB+E (own-history plus the
...
against HAR's 0.370 at $h1$), so a multi-hop graph network recovers nothing the own-history model does
...
own-history model, XGB+E adds the earnings block, and XGB+E+LG adds leaf-graph smoothing. Bold marks
...
\textbf{A diagnostic locates the missing lever.} The own-history XGBoost model beats HAR across the
...
one (Fig.~\ref{fig:decile-bias}). Permutation importance ranks the three own-history HAR lags far above
...
earnings feature the natural lever for the residual the own-history model cannot reach.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{fig_remaining_error_by_decile.png}
\caption{The own-history XGBoost model beats HAR on the calm deciles, and its remaining QLIKE concentrates in
...
\caption{The own-history XGBoost model under-forecasts the storm deciles, where its median forecast-to-realized ratio drops
below one.}
...
The gain decomposes into an own-history nonlinearity component (XGB over HAR) and an incremental earnings
...
the tree nonlinearity alone, drives the bulk of the S\&P~500 advantage.
...
does not beat a per-stock gradient-boosted tree model over own-history features. In place of a learned
...
horizons with 390{,}577 to 399{,}040 stock-day observations per horizon, the own-history XGBoost model
again wins, the earnings lever does not transfer, and a leaf-cooccurrence graph is a small but
significant positive lever.

\textbf{The own-history model wins on HOSE too.} Table~\ref{tab:hose} reports all four
metrics. The own-history XGBoost model is the DM-verified best of the non-graph models at every
...
loses to the XGBoost model at every horizon (QLIKE worse by 4.9 to 7.1\%, DM $p<10^{-27}$).
...
QLIKE at the three shortest horizons.
...
its estimable subset ($\sim$98.6\% of rows), a slightly different sample than the other models. XGB is the own-history model, XGB+E
...
10 & GNNHAR      & 0.628 & 0.792 & 0.483 & 1.767 \\ % gnnhar_hose.json GNNHAR_h10
10 & XGB         & 0.595 & 0.771 & 0.439 & 1.682 \\ % leaf_graph_paper_hose_noearn_h10.json metrics.XGB (n_folds=8; mse 5.9463e-7, rmse 7.711e-4, mae 4.389e-4, qlike 1.6816)
10 & XGB+E       & 0.595 & 0.771 & 0.441 & 1.684 \\ % leaf_graph_hose_h10.json metrics.XGB
10 & XGB+E+LG    & \textbf{0.595} & \textbf{0.771} & \textbf{0.439} & \textbf{1.682} \\ % leaf_graph_hose_h10.json metrics.XGB+leafgraph (DM +0.12% p=4.6e-3, spike-robust)
\midrule
...
22 & GNNHAR      & 0.655 & 0.809 & 0.507 & 1.810 \\ % gnnhar_hose.json GNNHAR_h22
22 & XGB         & 0.621 & 0.788 & \textbf{0.459} & \textbf{1.725} \\ % leaf_graph_paper_hose_noearn_h22.json metrics.XGB (n_folds=8; mse 6.2128e-7, rmse 7.882e-4, mae 4.591e-4, qlike 1.7251) -- own-history is min QLIKE/MAE at h22 (earnings inert)
...
The lever is own-history denoising, not an earnings effect: it survives the no-earnings ablation, where
XGB+E+LG over the own-history XGB gains 0.22\% at $h1$ (DM $p<0.001$) and 0.12\% at $h5$ (DM $p<0.001$).
% no-earn: leaf_graph_paper_hose_noearn_h{1,5}.json gain_pct / dm
The smoothing weight $\alpha$, fit per fold on validation, averages about 0.29 to 0.35 on HOSE, so the
...
within about 0.2\% of the own-history XGB at every horizon. A per-stock, per-day diagnostic on the HOSE
...
\textbf{The HOSE result is robust to regime spikes.} The own-history XGBoost model beats HAR at every
...
The own-history advantage is a calm-market property, not an artifact of a few crisis days.

\textbf{Takeaway.} The own-history XGBoost model wins on a second, structurally different market. The
...
\noindent\textbf{Earnings decomposition.} The S\&P~500 own-history-to-earnings decomposition (8.7 to
...
(XGBoost) over own-history features and asked where extra information helps. The own-history model beats
...
HOSE tickers under an identical protocol: the own-history model again wins and the earnings lever does
...
that, for daily volatility, a strong own-history model plus a market-appropriate external signal is what
