# Volatility forecasting — HAR / HARQ / GBM / GBME(GBM+Earning) / GBME+graph (all metrics, DM-bolded)

GBM = gamma-loss Gradient Boosting Machine (HistGradientBoostingRegressor) on own-history features; GBME(GBM+Earning) = GBM + External earnings features; GBME+graph adds the cross-firm correlation-graph neighbour aggregate. QLIKE is the primary loss (equal to the gamma deviance up to a constant); Diebold-Mariano tests use per-trading-date mean loss.
