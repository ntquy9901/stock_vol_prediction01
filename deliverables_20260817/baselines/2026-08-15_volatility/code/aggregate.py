...
Reads results/volatility_gat_seed{seed}_<TS>/ (ladder_metrics.json + per-obs test dumps in P0/NODE/GNN)
and writes results/volatility_gat_<TS>_summary.json + a markdown table.
...
def _run_dir(ts: str, seed: int) -> Path:
    return ROOT / "results" / f"volatility_gat_seed{seed}_{ts}"
...
def main(ts: str, seeds=(42, 123, 2026)) -> None:
    ...
    out_path = ROOT / "results" / f"volatility_gat_{ts}_summary.json"
