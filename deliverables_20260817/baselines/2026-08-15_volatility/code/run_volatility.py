...
Run (GPU venv):  python <.../code/run_volatility.py> <TS> [device] [epochs] [seeds...]
Writes results/volatility_gat_seed{seed}_<TS>/ladder_metrics.json + models/volatility_gat_seed{seed}_<TS>.pt.
"""
...
from model import POSITIVITY_EPSILON, VolatilityModel  # noqa: E402
...
def build_volatility_basis(stamp: Path) -> dict[str, Any]:
    ...
    print(f"[volatility] volume z-score window = {_eda_features._VOLUME_WINDOW} trading days", flush=True)
...
class _StoreShim:
    ...
    ``build_volatility_basis``, so this also lets the smoke test's fake basis (no real store) run
...
def main(ts: str, device_name: str = "cuda", seeds: tuple[int, ...] = (42,), epochs: int = 15,
    ...
    for seed in seeds:
        out_base = ROOT / "results" / f"volatility_gat_seed{seed}_{ts}"
        t0 = time.perf_counter()
        ladder = run_seed(seed, epochs, ts, out_base, device, resume=resume)
        ckpt = out_base / "ckpt.pt"
        target = models_dir / f"volatility_gat_seed{seed}_{ts}.pt"
        if ckpt.exists():
