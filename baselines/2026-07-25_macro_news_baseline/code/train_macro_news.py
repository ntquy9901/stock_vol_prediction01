"""Train the macro-augmented dual-group news baseline (HAR + dual-group news + market-wide
macro news branches, concatenated).

Mirrors `2026-07-25_dual_group_news_embedding_baseline/code/train_dual_news.py` exactly (same
4-tuple batch shape, same model class `DualGroupNewsBaseline` reused UNCHANGED with a wider
`n_feat` — see design.md §5) — only the dataloader factory differs
(`create_macro_news_dataloaders` instead of `create_dual_news_dataloaders`).

Training policy (CLAUDE.md): default 10 epochs for this first experimental run; report every 5.
User unavailable overnight to approve >10 epochs — capped at 10 regardless of in-progress results.

Run (smoke, no panels needed — dataset falls back to dummy n_feat):
  python train_macro_news.py --epochs 2 --smoke

Real run (after build_macro_panel.py + the sibling's build_dual_group_panel.py have run):
  python train_macro_news.py --epochs 10
"""
import sys
import argparse
import json
...
from model_dual_news import DualGroupNewsBaseline  # noqa: E402 (sibling, read-only, unchanged)
from dataset_macro_news import create_macro_news_dataloaders  # noqa: E402
...
def main():
    ...
    ap.add_argument("--macro_panel_path",
                    default=str(_ROOT / "data" / "features" / "macro_news_panel.parquet"))
    ap.add_argument("--epochs", type=int, default=10,
                    help="CLAUDE.md Training policy: default 5-10 epochs for experimentation")
    ...
    ap.add_argument("--smoke", action="store_true",
                    help="quick run (no real panels needed; verifies shapes/forward)")
    ap.add_argument("--graph_method", default="knn")
    args = ap.parse_args()

    if args.epochs > 10:
        raise ValueError(
            "CLAUDE.md Training policy: >10 epoch requires explicit user approval. This baseline "
            "was run unattended overnight — capped at 10. Re-run with a smaller --epochs, or get "
    ...
    news_panel_path = args.news_panel_path if not args.smoke else None
    macro_panel_path = args.macro_panel_path if not args.smoke else None
    train_loader, val_loader, test_loader, (train_ds, val_ds, test_ds) = \
        create_macro_news_dataloaders(
            data_dir=args.data_dir, news_panel_path=news_panel_path,
            macro_panel_path=macro_panel_path, graph_method=args.graph_method,
    ...
    out_dir = _ROOT / "results" / f"macro_news_{timestamp}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ckpt_dir = _ROOT / "models" / f"macro_news_{timestamp}"
    ...
    results = {
        "model": "MacroNewsBaseline (DualGroupNewsBaseline, wider n_feat)",
