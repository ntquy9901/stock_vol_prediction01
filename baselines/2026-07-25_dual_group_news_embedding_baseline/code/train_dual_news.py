"""Train the dual-group news embedding baseline (HAR + pre-aggregated dual-group news branch).

Custom train_epoch/validate to handle the 4-tuple batch (x_har, adj, x_news, y) and the model's
forward(x_har, adj, x_news). Reuses EarlyStopping + evaluate_predictions + VolatilityNormalizer
(read-only imports). Loss: MSE on normalized scale. Metrics: 6 mandatory, on denormalized scale.
Mirrors `2026-07-07_embedding_baseline/code/train_embedding_baseline.py` (no mask tensor needed
here — see model_dual_news.py docstring).

Training policy (CLAUDE.md): default 10 epochs for this first experimental run; report every 5.

Run (smoke, no panel needed — dataset falls back to dummy n_feat=146):
  python baselines/2026-07-25_dual_group_news_embedding_baseline/code/train_dual_news.py --epochs 2 --smoke

Real run (after build_dual_group_panel.py produced data/features/dual_group_news_panel.parquet):
  python baselines/2026-07-25_dual_group_news_embedding_baseline/code/train_dual_news.py --epochs 10
"""
import sys
import argparse
import json
...
from dataset_dual_news import create_dual_news_dataloaders
...
def main():
    ...
    out_dir = _ROOT / "results" / f"dual_group_news_{timestamp}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ckpt_dir = _ROOT / "models" / f"dual_group_news_{timestamp}"
