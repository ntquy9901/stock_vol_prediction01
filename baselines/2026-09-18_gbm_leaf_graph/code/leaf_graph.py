...
with ``alpha`` fit on a validation slice per fold, frozen for test. The leaf-graph is a deterministic
re-encoding of the SAME own-history features the GBM already used, so it carries no new information; this module
exists to measure — honestly — whether smoothing over it helps per-stock QLIKE (expected: no; alpha -> 0).
