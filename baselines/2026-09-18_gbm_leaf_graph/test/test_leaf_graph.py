"""Tests for the GBME + leaf-cooccurrence-graph falsification baseline: leaf-Hamming similarity, kNN
neighbour-mean, alpha-smoothing (identity at 0, neighbour-mean at 1, monotone, causal per-day), booster
plumbing + numerical guard, DM/verdict/spike logic, and a walk-forward run smoke carrying the gate-required
over/under-fit evidence keys. Synthetic panels keep the driver fast without real data."""
import numpy as np
import pandas as pd
...
def test_predict_xgb_seed_ensemble_positive():
    ...
    p = LG.predict_xgb(tr, combo, ["f0", "f1"], (0, 1))
...
def test_run_hose_structure_evidence_perfold_spike(monkeypatch, tmp_path):
    ...
    assert set(docs) == {1}
    doc = docs[1]
    assert (tmp_path / "leaf_graph_hose_smoke_h1.json").exists()
    for blk in ("metrics", "train_metrics", "val_metrics"):
        for m in R.ORDER:
            assert set(doc[blk][m]) == {"mse", "rmse", "mae", "r2", "qlike"}
    assert set(doc["dm"]) == {"XGB+leafgraph_vs_XGB", "XGB+leafgraph_vs_GBME"}
...
def test_run_sp500_no_spike_block(monkeypatch, tmp_path):
    ...
    assert (tmp_path / "leaf_graph_sp500_smoke_h1.json").exists()
