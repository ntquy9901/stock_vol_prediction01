...
import dm_retrain as dmr  # noqa: E402
...
def test_dm_pair_reads_retrain_dirs_and_favors_closer(tmp_path, monkeypatch):
    monkeypatch.setattr(dmr, "RESULTS", tmp_path)
    rng = np.random.default_rng(0)
    targets = np.abs(rng.normal(1.0, 0.2, 40)) + 0.1
    ts, h, seeds = "T", 5, [42]
    run = tmp_path / f"volatility_retrain_h{h}_seed42_{ts}"   # retrain dir prefix
    ...
    r = dmr.dm_pair(ts, h, "FULL", "HAR", seeds)
    assert r["n"] == 40
    assert r["favors"] == "A"          # FULL closer -> lower QLIKE
    assert r["mean_diff"] < 0
    for loss in ("se", "ae"):
        rl = dmr.dm_pair(ts, h, "FULL", "HAR", seeds, loss=loss)
...
def test_ensemble_averages_seeds(tmp_path, monkeypatch):
    ...
    for s, bias in ((42, 0.0), (123, 0.4)):
        run = tmp_path / f"volatility_retrain_h{h}_seed{s}_{ts}"
        _write_dump(run / "FULL" / "predictions_test.json", targets, targets + bias)
    keys, tgt, pred = dmr._ensemble(ts, h, "FULL", [42, 123])
    assert len(keys) == 3
