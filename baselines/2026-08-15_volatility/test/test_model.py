...
def test_forward_shape_and_graph_changes_output():
    torch.manual_seed(0)
    m = VolatilityModel(price_dim=5, news_dim=146, num_tickers=4)
...
def test_ablation_flags_lstm_only_and_no_gate():
    ...
    lstm = VolatilityModel(5, 146, 4, use_news=False, use_gate=False)
    ...
    news = VolatilityModel(5, 146, 4, use_news=True, use_gate=False)
...
def test_use_graph_false_drops_gat_branch():
    torch.manual_seed(0)
    b = _batch()
    no_graph = VolatilityModel(5, 146, 4, use_graph=False)
