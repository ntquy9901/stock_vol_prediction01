"""Train the REST-TS baseline (residual-exclusive supervision, arXiv:2606.19413).

Custom train_epoch/validate: loss = MSE(har_pred, y) + MSE(news_pred, (y-har_pred).detach()).
The .detach() is the crux of REST-TS — it stops gradient from the residual loss flowing back
into har_pred, so har_head trains ONLY on the primary task and news_head trains ONLY on the
part HAR could not explain. Final prediction/metrics use (har_pred + news_pred).

Reuses create_embedding_dataloaders (sibling 2026-07-07, read-only import) — same data as the
original embedding baseline, so results are directly comparable (fusion mechanism is the only
variable that changed).

Run (smoke): python .../train_resttext.py --epochs 2 --smoke
Real run:    python .../train_resttext.py --epochs 10
"""
import sys
import argparse
import json
...
def train_epoch(model, loader, criterion, optimizer, device, grad_clip=1.0):
    ...
    for x_har, adj, x_emb, mask, y in loader:
        ...
        B, S = y.shape
        optimizer.zero_grad()
        har_pred, news_pred = model(x_har, adj, x_emb, mask)   # [B,S] each
        loss_har = criterion(har_pred.reshape(B * S), y.reshape(B * S))
        residual_target = (y - har_pred).detach()               # [REST-TS] stop-gradient
        loss_news = criterion(news_pred.reshape(B * S), residual_target.reshape(B * S))
...
def validate(model, loader, criterion, device, dataset):
    ...
    with torch.no_grad():
        for x_har, adj, x_emb, mask, y in loader:
            ...
            combined = har_pred + news_pred
            preds_n.append(combined.cpu().numpy().reshape(-1))
...
def main():
    ...
    model = RestTsBaseline(config, emb_dim=emb_dim, d_news=args.d_news,
    ...
    out_dir = _ROOT / "results" / f"resttext_{timestamp}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ckpt_dir = _ROOT / "models" / f"resttext_{timestamp}"
