...
def test_aggregate_dm_and_table(tmp_path, monkeypatch, capsys):
    ...
    seeds = (42, 123, 2026)
    for seed in seeds:
        run = tmp_path / "results" / f"trackA_gat_seed{seed}_{_TS}"
    ...
    summary = json.loads((tmp_path / "results" / f"trackA_gat_{_TS}_summary.json").read_text())
...
def test_aggregate_degenerate_pair_recorded_not_crash(tmp_path, monkeypatch):
    ...
    dm = json.loads((tmp_path / "results" / f"trackA_gat_{_TS}_summary.json").read_text())["diebold_mariano"]
