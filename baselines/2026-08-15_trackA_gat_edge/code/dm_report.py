"""Diebold-Mariano (HLN) verdicts for the trackA leave-one-out study.

Reads per-observation held-out test prediction dumps from
``results/trackA_ablation_h{h}_seed{s}_<TS>/{dump_dir}/predictions_test.json``, optionally
...
def _ensemble(ts: str, horizon: int, rung: str, seeds) -> tuple[list, np.ndarray, np.ndarray]:
    per_seed = [_load_rows(RESULTS / f"trackA_ablation_h{horizon}_seed{s}_{ts}" /
