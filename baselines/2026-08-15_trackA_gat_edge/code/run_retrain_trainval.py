...
Writes results/trackA_retrain_h{h}_seed{seed}_<TS>/retrain_metrics.json + per-rung test dumps.
"""
...
from run_trackA import ROOT, _StoreShim, _evaluate_rung  # noqa: E402
...
def _build_deep_basis(graph, store) -> dict[str, Any]:
    """Snapshot dicts + per-ticker scaler tensors + dims, mirroring run_trackA.build_trackA_basis."""
...
def run_horizon(horizon: int, seed: int, epochs: int, ts: str, device: torch.device) -> dict[str, Any]:
    ...
    features._VOLUME_WINDOW = 22                                # unified 22-day window (as build_trackA_basis)
    out_base = ROOT / "results" / f"trackA_retrain_h{horizon}_seed{seed}_{ts}"
