"""Train the Top-3 News Gate baseline (HAR + news branch, masked to just VIB/ACB/MWG).
...
`Top3NewsGateBaseline` (3-ticker allowlist instead of the 22/10 classification).

Training policy (CLAUDE.md): default 10 epochs for this first experimental run.

Run (smoke, no panel needed):
  python baselines/2026-07-25_top3_news_gate_baseline/code/train_top3_gate.py --epochs 2 --smoke
...
from model_top3_gate import NEWS_ON_TICKERS, Top3NewsGateBaseline
...
def main():
    ...
    model = Top3NewsGateBaseline(config, n_feat=n_feat, stock_names=train_ds.stock_names,
    ...
    out_dir = _ROOT / "results" / f"top3_gate_{timestamp}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ckpt_dir = _ROOT / "models" / f"top3_gate_{timestamp}"
