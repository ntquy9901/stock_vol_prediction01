"""Walk-forward driver: does a Gamma-GLM base-margin anchor + XGBoost gamma residual (Hướng 3) beat the champion
own-history gamma-GBM (GBME) on out-of-sample QLIKE?

Per horizon h and outer fold k: fit GBME (canonical HGBR gamma), XGB (plain XGBoost gamma), and GLM+XGB (XGBoost
gamma with a Gamma-GLM log-link base margin) on the causal train window (minus a val slice), score train/val/test,
pool over folds, date-clustered DM (GLM+XGB vs GBME = beat-the-champion; GLM+XGB vs XGB = isolate the anchor),
verdict + HOSE per-fold QLIKE + regime-spike robustness.

Output: results/gamma_gbm/glm_anchor_<market>_h<h>.json, one per horizon, atomic checkpoint after every fold.

Run: python run_glm_anchor.py [hose|sp500] [--smoke]
"""
...
import glm_anchor as GA  # noqa: E402

FL = FM.FL
GBME, XGB, GLMX = "GBME", "XGB", "GLM+XGB"
ORDER = [GBME, XGB, GLMX]
...
def _predict(model, trf, combo, cols, seeds):
    """Seed-ensembled prediction for one model over the ``combo`` rows."""
    if model == GBME:
        return np.mean([FM.gbm(trf, combo, cols, s) for s in seeds], 0)
    if model == XGB:
        return GA.predict_xgb(trf, combo, cols, seeds)
    return GA.predict_glm_xgb(trf, combo, cols, seeds)
...
def success(docs):
    """Pre-registered: GLM+XGB beats GBME at BOTH KILL_HORIZONS. Missing horizon = fail."""
...
def _pool_doc(h, market, seeds, preds, yy, dts, per_fold, spike):
    ...
    (GLM+XGB vs GBME primary, vs XGB secondary), gain, verdict, plus HOSE per-fold/spike robustness."""
    ...
    dm_gbme = _safe_dm(err[GLMX], err[GBME], dates, h)
    dm_xgb = _safe_dm(err[GLMX], err[XGB], dates, h)
    gain = (metrics[GBME]["qlike"] - metrics[GLMX]["qlike"]) / metrics[GBME]["qlike"] * 100.0
    doc = {"market": market, "h": h, "n": int(len(y)), "n_folds": len(yy["te"]), "seeds": list(seeds),
           ...
           "fit_diagnostics": fit,
           "dm": {f"{GLMX}_vs_{GBME}": dm_gbme["p_value"], f"{GLMX}_vs_{XGB}": dm_xgb["p_value"]},
           "gain_pct": {f"{GLMX}_vs_{GBME}": gain,
                        f"{GLMX}_vs_{XGB}": (metrics[XGB]["qlike"] - metrics[GLMX]["qlike"])
                        / metrics[XGB]["qlike"] * 100.0},
           "verdict": {"beats": verdict(gain, dm_gbme["p_value"]), "gain_pct": gain, "dm_p": dm_gbme["p_value"],
                       "criterion": "GLM+XGB beats GBME: gain>GAIN_MIN AND DM p<DM_ALPHA"}}
    if per_fold:
    ...
    if spike:
        ...
        keep = ~sm
        if keep.any():
            dm_ex = _safe_dm(err[GLMX][keep], err[GBME][keep], dates[keep], h)
            qb, qz = float(np.mean(err[GBME][keep])), float(np.mean(err[GLMX][keep]))
            doc["spike_robustness"] = {
                "n_spike_obs": int(sm.sum()), "n_ex_spike_obs": int(keep.sum()),
                "qlike_ex_spike": {GBME: qb, GLMX: qz}, "gain_pct_ex_spike": (qb - qz) / qb * 100.0,
...
def run(market, load_fn=None, out_dir=None, smoke=False):
    """Walk-forward GBME vs XGB vs GLM+XGB for a market. Writes one JSON per horizon and returns {h: doc}."""
    ...
    for h in horizons:
        t0 = time.time()
        out_path = out_dir / f"glm_anchor_{market}{tag}_h{h}.json"
        ...
        for k in range(len(S1.FOLDS) - 1):
            ...
            for m in ORDER:
                p = _predict(m, trf_e, combo, cols, seeds)
        ...
        for m in ORDER:
            print(f"  {m:9s} QLIKE {doc['metrics'][m]['qlike']:.4f}", flush=True)
        print(f"  DM {GLMX} vs {GBME}: {v['gain_pct']:+.2f}% (p={v['dm_p']:.4f}) beats={v['beats']} | "
              f"vs {XGB}: p={doc['dm'][f'{GLMX}_vs_{XGB}']:.4f} | saved {out_path.name}", flush=True)
        docs[h] = doc
    return docs
