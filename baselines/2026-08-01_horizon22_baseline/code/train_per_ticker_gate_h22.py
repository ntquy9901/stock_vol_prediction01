"""Train the per-ticker gated-news baseline at a 22-DAY-AHEAD horizon (~1 trading month), for
comparison against the project's 5-day and 10-day results.

Hard-isolated copy-modification of
`2026-08-01_horizon10_baseline/code/train_per_ticker_gate_h10.py` (itself a copy-modification of
...
debug/gate-history/plotting logic, same resume-checkpoint support (needed today's 5-day experiment
to extend past epoch 10). The ONLY change: `--forecast_horizon` default 22 (vs. 10).

Run (smoke, no panel needed):
  python train_per_ticker_gate_h22.py --epochs 2 --smoke

Real run:
  python train_per_ticker_gate_h22.py --epochs 10 --forecast_horizon 22
"""
import sys
import argparse
import json
...
def load_resume_state(resume_results_dir: Path | None) -> tuple[dict, list, list, int]:
    """Same contract as the sibling scripts' `load_resume_state` -- loads a previous run's
    `gate_history.json` (+ `loss_history.json` if present) so a resumed run's epoch numbering and
    gate table continue where the previous run left off."""
...
def plot_gate_evolution(gate_history: dict, stock_names: list, out_path: Path):
    ...
    ax.set_title("Per-ticker news gate evolution (22-day-ahead horizon)")
...
def main():
    ...
    ap.add_argument("--forecast_horizon", type=int, default=22,
                    help="days ahead to predict (this baseline: 22, ~1 trading month)")
