"""Train a HAR-only reference model (ParallelLSTMGNN) at a 22-DAY-AHEAD horizon (~1 trading
month, matches HAR "monthly" window), for comparison against the project's 5-day and 10-day
results.

Hard-isolated copy-modification of
`2026-08-01_horizon10_baseline/code/train_har_only_reference_h10.py` (itself a copy-modification
of `2026-07-25_news_usefulness_ablation/code/train_har_only_reference.py`) — CLAUDE.md §3.F rule
3. The ONLY change: `--forecast_horizon` default 22 (vs. 10). `forecast_horizon` is already a
plain constructor parameter in the shared dataset pipeline, no dataset/model code changes needed.

Run: python train_har_only_reference_h22.py --epochs 10 --forecast_horizon 22
"""
import sys
import argparse
import json
