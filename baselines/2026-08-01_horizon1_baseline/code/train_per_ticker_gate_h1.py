"""Train the per-ticker gated-news baseline at a 1-DAY-AHEAD horizon, for comparison against the
project's 5-day, 10-day, and 22-day results (completes the "1, 10, 22-day Secondary Targets" set
listed in CLAUDE.md).

Hard-isolated copy-modification of
`2026-08-01_horizon22_baseline/code/train_per_ticker_gate_h22.py` (itself a copy-modification of
`2026-08-01_horizon10_baseline/code/train_per_ticker_gate_h10.py`, in turn from
...
debug/gate-history/plotting logic, same resume-checkpoint support. The ONLY change:
`--forecast_horizon` default 1 (vs. 22).

Run (smoke, no panel needed):
  python train_per_ticker_gate_h1.py --epochs 2 --smoke

Real run:
  python train_per_ticker_gate_h1.py --epochs 10 --forecast_horizon 1
"""
import sys
import argparse
import json
...
def plot_gate_evolution(gate_history: dict, stock_names: list, out_path: Path):
    ...
    ax.set_title("Per-ticker news gate evolution (1-day-ahead horizon)")
