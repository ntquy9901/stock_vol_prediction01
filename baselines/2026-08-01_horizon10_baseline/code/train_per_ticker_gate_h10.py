"""Train the per-ticker gated-news baseline at a 10-DAY-AHEAD horizon, for comparison against the
project's 5-day-ahead default (`2026-07-26_per_ticker_news_gate_baseline`, current best QLIKE/R²).
...
Same `PerTickerGatedNewsBaseline` model (imported UNCHANGED, read-only), same MSE loss, same
debug/gate-history/plotting logic, same 10-epoch training-policy cap. The ONLY change: a
`--forecast_horizon` CLI arg (default 10, vs. that script's implicit 5) threaded into
`create_dual_news_dataloaders(...)` -- `forecast_horizon` is already a plain constructor parameter
in the shared dataset pipeline (design.md §1), no dataset/model code changes needed.

Run (smoke, no panel needed):
  python train_per_ticker_gate_h10.py --epochs 2 --smoke

Real run:
  python train_per_ticker_gate_h10.py --epochs 10 --forecast_horizon 10
"""
import sys
import argparse
import json
...
def load_resume_state(resume_results_dir: Path | None) -> tuple[dict, list, list, int]:
    """Same contract as the 5-day sibling's `load_resume_state` -- loads a previous run's
    `gate_history.json` (+ `loss_history.json` if present) so a resumed run's epoch numbering and
    gate table continue where the previous run left off (used to extend this h10 run from 10 to
    20 epochs, per Training Policy explicit-approval path, 2026-08-01)."""
...
def plot_gate_evolution(gate_history: dict, stock_names: list, out_path: Path):
    ...
    ax.set_title("Per-ticker news gate evolution (10-day-ahead horizon)")
...
def main():
    ...
    ap.add_argument("--forecast_horizon", type=int, default=10,
                    help="days ahead to predict (this baseline: 10, vs. sibling's default 5)")
    ...
    out_dir = _ROOT / "results" / f"per_ticker_gate_h{args.forecast_horizon}_{timestamp}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ckpt_dir = _ROOT / "models" / f"per_ticker_gate_h{args.forecast_horizon}_{timestamp}"
