"""Train a HAR-only reference model (ParallelLSTMGNN) at a 10-DAY-AHEAD horizon, for comparison
against the project's 5-day-ahead default.
...
The ONLY change: a `--forecast_horizon` CLI arg (default 10, vs. that script's implicit 5) threaded
into `create_dual_news_dataloaders(...)`. Everything else (model, loss, optimizer, evaluation) is
identical -- `forecast_horizon` is already a plain constructor parameter in the shared dataset
pipeline (design.md §1), so no dataset/model code changes are needed at all.

Reuses `create_dual_news_dataloaders` (sibling baseline, read-only) purely for its x_har/adj/y
construction -- `news_panel_path=None` means x_news is a dummy all-zero tensor this script never
looks at (only x_har, adj, y are used).

Run: python train_har_only_reference_h10.py --epochs 10 --forecast_horizon 10
"""
import sys
import argparse
import json
