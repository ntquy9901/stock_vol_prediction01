"""Train the per-ticker gate baseline on calendar-augmented news features.

Hard-isolated copy-modification of
`2026-07-26_per_ticker_news_gate_baseline/code/train_per_ticker_gate.py` (CLAUDE.md §3.F rule 3).
Same `PerTickerGatedNewsBaseline` model (imported UNCHANGED, read-only, from that sibling), same
MSE loss, same debug/gate-history/plotting logic, same 10-epoch training-policy cap. The ONLY
change: dataloaders come from THIS baseline's `create_calendar_news_dataloaders`
(`dataset_calendar_news.py`), whose `x_news` is 10 columns wider (146 dual-group + 10 calendar,
see requirements.md §3) than the sibling's 146-column `x_news`. This isolates exactly one
experimental variable: does adding calendar features change the metrics, holding gate mechanism,
epoch budget, loss, and optimizer config fixed (design.md §6)?

Run (smoke, no panel needed):
  python train_calendar_news_gate.py --epochs 2 --smoke

Real run:
  python train_calendar_news_gate.py --epochs 10
"""
import sys
import argparse
import json
...
from dataset_calendar_news import create_calendar_news_dataloaders
from calendar_features import CALENDAR_FEATURE_GROUPS
...
def print_gate_table(stock_names, gate_values, prev_gate_values, epoch):
    """Console debug output (same convention as per_ticker_gate sibling): all tickers sorted by
    gate value desc, with delta vs. the previous epoch."""
...
def load_resume_state(resume_results_dir: Path | None) -> tuple[dict, list, list, int]:
    """Identical contract to the sibling's `load_resume_state` (see that file's docstring)."""
...
def plot_gate_evolution(gate_history: dict, stock_names: list, out_path: Path):
    """1 line per ticker (colormap), x=epoch, y=gate value. Companion to the mandatory loss
    learning curve (CLAUDE.md §3.C), same 5-epoch cadence, but for the gate instead of the loss."""
    ...
    ax.set_title("Per-ticker news gate evolution (calendar-augmented x_news)")
...
def main():
    ...
    ap.add_argument("--gate_lr", type=float, default=0.05,
                    help="LR for gate_logits (higher than --lr on purpose, see sibling design.md §2)")
    ...
    ap.add_argument("--calendar_groups", default="all", choices=sorted(CALENDAR_FEATURE_GROUPS),
                    help="which calendar feature GROUP to include (2026-08-01 ablation: isolate "
                         "which group, if any, carries signal -- see calendar_features.py)")
    ...
    calendar_feature_names = CALENDAR_FEATURE_GROUPS[args.calendar_groups]
    print(f"[train] device={device}, smoke={args.smoke}, gate_lr={args.gate_lr}, lr={args.lr}, "
          f"calendar_groups={args.calendar_groups} ({calendar_feature_names})")
    ...
    train_loader, val_loader, test_loader, (train_ds, val_ds, test_ds) = \
        create_calendar_news_dataloaders(
            data_dir=args.data_dir, news_panel_path=panel_path, graph_method=args.graph_method,
            batch_size=args.batch_size, config=config, calendar_feature_names=calendar_feature_names)
    ...
    out_dir = _ROOT / "results" / f"calendar_gate_{args.calendar_groups}_{timestamp}"
    out_dir.mkdir(parents=True, exist_ok=True)
    ckpt_dir = _ROOT / "models" / f"calendar_gate_{args.calendar_groups}_{timestamp}"
    ...
    results = {
        "model": "PerTickerGatedNewsBaseline (calendar-augmented x_news)",
        "calendar_groups": args.calendar_groups, "calendar_feature_names": calendar_feature_names,
