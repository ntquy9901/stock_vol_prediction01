"""Calendar-augmented dual-group news dataset: returns (x_har, adj, x_news, y).

Mirrors `2026-07-25_macro_news_baseline/code/dataset_macro_news.py`'s pattern (same repo
convention: each baseline's dataset is a focused copy+extension of the previous one) -- but
where that baseline concatenates a PhoBERT/PCA-derived macro vector loaded from a pre-built
parquet panel, this one concatenates a `calendar_features.compute_calendar_vector(date)` vector
computed ON THE FLY (no panel/parquet needed -- see design.md §2: calendar features are a cheap
pure function of the date string, unlike macro news which requires PhoBERT+PCA).

Isolated -- no edit to src, no edit to the sibling baselines. `load_news_panel` is imported
read-only from the `2026-07-25_dual_group_news_embedding_baseline` sibling (per-ticker dual-group
panel loader).
"""
import sys
...
N_CALENDAR_FEAT = len(CALENDAR_FEATURE_NAMES)


class MultiStockDatasetWithCalendarNews(MultiStockDatasetWithPreSplitData):
    """Dataset returning (x_har, adj, x_news, y), x_news = dual-group vec ++ calendar vec.

    x_har : [seq, stocks, 3]                          (normalized in __getitem__)
    x_news: [seq, stocks, n_dual_feat + n_calendar_feat]  (dual-group NOT normalized, already
            PCA/EWMA-scaled upstream; calendar features are already bounded/cyclical, no
            normalization needed)
    y     : [stocks]                                   (normalized in __getitem__)

    `calendar_feature_names`: subset of CALENDAR_FEATURE_NAMES to include (default: all 10) --
    lets the ablation study (2026-08-01, "which calendar GROUP carries signal") reuse this exact
    dataset/model/train pipeline with only e.g. the 2 Tet columns, or only the 2 earnings columns,
    instead of duplicating the whole class per group (see calendar_features.CALENDAR_FEATURE_GROUPS).
    """

    def __init__(self, *args, news_panel_path=None, calendar_feature_names=None, **kwargs):
        ...
        names = list(calendar_feature_names) if calendar_feature_names is not None else list(CALENDAR_FEATURE_NAMES)
        unknown = set(names) - set(CALENDAR_FEATURE_NAMES)
        if unknown:
            raise ValueError(f"unknown calendar feature name(s): {unknown}")
        self._calendar_feature_names = names
        self._calendar_indices = [CALENDAR_FEATURE_NAMES.index(n) for n in names]

        # per-date calendar vector cache -- avoids recomputing the same date's features once per
        # ticker (30x redundant calls per window otherwise, since calendar features don't depend
        # on ticker at all)
        self._calendar_cache: dict[str, np.ndarray] = {}
        super().__init__(*args, **kwargs)

    def _calendar_vec(self, date_str: str) -> np.ndarray:
        vec = self._calendar_cache.get(date_str)
        if vec is None:
            full = compute_calendar_vector(date_str)
            vec = full[self._calendar_indices]
            self._calendar_cache[date_str] = vec
        return vec
    ...
    def _create_sequences(self):
        ...
        self._n_feat = n_dual + len(self._calendar_feature_names)
        if not self._news_feature_cols:
            print(f"[calendar_news] no dual-group panel found -> dummy n_dual={n_dual} (smoke mode)")
        ...
        for i in range(min_length - self.seq_length - self.forecast_horizon):
            ...
            for stock_name in self.stock_names:
                ...
                for d in window_dates:
                    ...
                    cal_vec = self._calendar_vec(d)
                    day_feats.append(np.concatenate([dual_vec, cal_vec]))
        ...
        print(f"[calendar_news] date-match coverage: dual={self._matched_dual_cells}/"
              f"{self._total_cells} ({cov_dual:.2f}%); calendar=100.00% (pure function, always "
              f"defined)")
...
def create_calendar_news_dataloaders(
    ...
    remove_outliers=True, n_std=3.0, config=None, calendar_feature_names=None,
):
    """Mirror of create_dual_news_dataloaders, using MultiStockDatasetWithCalendarNews. No edits
    to any sibling/original file."""
    ...
    print(f"\n[create_calendar_news_dataloaders] graph_method={graph_method}, "
    ...
    print(f"[calendar_news] common stocks: {len(common_stocks)}")
    ...
    ds_kwargs = dict(seq_length=seq_length, forecast_horizon=forecast_horizon,
                     ...
                     news_panel_path=news_panel_path, calendar_feature_names=calendar_feature_names)
    train_ds = MultiStockDatasetWithCalendarNews(train_raw_c, train_har_c, common_stocks,
                                                 train_mode=True, **ds_kwargs)
    val_ds = MultiStockDatasetWithCalendarNews(val_raw_c, val_har_c, common_stocks,
                                               train_mode=False, **ds_kwargs)
    test_ds = MultiStockDatasetWithCalendarNews(test_raw_c, test_har_c, common_stocks,
                                                train_mode=False, **ds_kwargs)
    print(f"[calendar_news] sequences - train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}")
    print(f"[calendar_news] detected n_feat={train_ds._n_feat}")
    ...
    print("[create_calendar_news_dataloaders] COMPLETE")
