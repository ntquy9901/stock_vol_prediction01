"""
LSTM-HAR Training Script with 3-Way Temporal Split

This module trains the LSTM-HAR model using:
...
def train_lstm_har_with_val(data_dir: str, output_dir: str = None):
    """
    Train LSTM-HAR with 3-way temporal split (70/15/15).

    Uses HAR features (daily, weekly, monthly) instead of raw volatility.

    Args:
        data_dir: Directory containing processed CSV files
        output_dir: Output directory (default: results/lstm_har_val_YYYY-MM-DD_HHMMSS)
    """
    if output_dir is None:
        timestamp = datetime.now().strftime("%Y-%m-%d_%H%M%S")
        output_dir = f'results/lstm_har_val_{timestamp}'

    print("=" * 80)
    print("LSTM-HAR TRAINING - 3-WAY TEMPORAL SPLIT (70/15/15)")
    print("=" * 80)
    ...
    # Initialize LSTM-HAR model
    print("\n4. Initializing LSTM-HAR model...")

    model = HARVolatilityLSTM(hidden_size=config['hidden_size'],
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'val_loss': val_loss,
            }, os.path.join(output_dir, 'best_lstm_har_val_model.pth'))
        else:
    ...
    checkpoint = torch.load(os.path.join(output_dir, 'best_lstm_har_val_model.pth'))
    ...
    with open(os.path.join(output_dir, 'lstm_har_val_results.json'), 'w') as f:
...
def plot_learning_curves(train_losses, val_losses, output_dir, epoch):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss (MSE)')
    plt.title('LSTM-HAR Learning Curves (with Validation)')
    plt.legend()
    ...
    if len(val_losses) > 10:
        ...
        if all(recent_val_trend[i] > recent_val_trend[i-1] for i in range(1, len(recent_val_trend))):
            print("[WARNING] Validation loss increasing - possible overfitting!")
...
if __name__ == "__main__":
    ...
    # Train LSTM-HAR with validation
