"""
CryptoMamba Training Script (v2) - Full Architecture

Based on original CryptoMamba with proper hyperparameters:
- Learning rate: 0.01 (10× higher)
- Max epochs: 1000 (10× longer)
- Weight decay: 0.0005 (500× stronger)
- Hidden dim: 14 (original setting)
...
from src.cryptomamba_baseline.model_v2 import create_cryptomamba_model_v2
...
from src.cryptomamba_baseline.config_v2 import (
...
def train_cryptomamba_v2(data_dir: str, output_dir: str = None):
    """
    Train CryptoMamba v2 with original hyperparameters and 3-way temporal split.

    Args:
        data_dir: Directory containing processed CSV files
        output_dir: Output directory (default: results/cryptomamba_v2_YYYY-MM-DD_HHMMSS)
    """
    if output_dir is None:
        timestamp = datetime.now().strftime("%Y-%m-%d_%H%M%S")
        output_dir = f'results/cryptomamba_v2_{timestamp}'
    ...
    print("  - Weight decay: 1e-6 → 0.0005 (500× stronger)")
    print("  - Hidden dim: 64 → 14 (original setting)")
    print("  - Full Mamba architecture (hierarchical)")
    ...
    print(f"  Hidden Dim: {CRYPTOMAMBA_CONFIG_V2['hidden_dim']} (original)")
    print(f"  Num Layers: {CRYPTOMAMBA_CONFIG_V2['num_layers']} (hierarchical)")
    print(f"  Dropout: {CRYPTOMAMBA_CONFIG_V2['dropout']} (original)")
    ...
    dataset = CryptoMambaDataset(
        ...
        forecast_horizon=CRYPTOMAMBA_CONFIG_V2['forecast_horizon'],
        use_volume=CRYPTOMAMBA_CONFIG_V2['use_volume'],
    )
    ...
    train_loader, val_loader, test_loader = splitter.create_dataloaders(
        dataset,
        batch_size=CRYPTOMAMBA_CONFIG_V2['batch_size'],
    ...
    # Initialize CryptoMamba model v2
    print("\n4. Initializing CryptoMamba model v2 (full architecture)...")
    model = create_cryptomamba_model_v2(**MODEL_CONFIG_V2)
    ...
    optimizer = torch.optim.Adam(
        model.parameters(),
        lr=TRAINING_CONFIG_V2['learning_rate'],
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer,
        step_size=TRAINING_CONFIG_V2['lr_step_size'],
        gamma=TRAINING_CONFIG_V2['lr_gamma']
    )

    # Training loop with validation
    print("\n5. Training with validation (original hyperparameters)...")
    num_epochs = TRAINING_CONFIG_V2['num_epochs']
    patience = TRAINING_CONFIG_V2['patience']
    min_epochs = TRAINING_CONFIG_V2['min_epochs']
    ...
    print(f"Learning rate scheduling: StepLR (step={TRAINING_CONFIG_V2['lr_step_size']}, gamma={TRAINING_CONFIG_V2['lr_gamma']})")
    epoch_times = []
    best_epoch = 0
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            }, os.path.join(output_dir, 'best_cryptomamba_v2_model.pth'))
        else:
    ...
    checkpoint = torch.load(os.path.join(output_dir, 'best_cryptomamba_v2_model.pth'))
    ...
    results = {
        'model': 'CryptoMamba V2 (Full Architecture, Original Hyperparameters)',
        'timestamp': timestamp,
        'phase': 'Phase 1 Proof-of-Concept',
        'features': f"HAR ({MODEL_CONFIG_V2['num_features']} features)",
        'split_method': 'Temporal (70/15/15)',
        'configuration': {
            'hidden_dim': int(CRYPTOMAMBA_CONFIG_V2['hidden_dim']),
            'num_layers': int(CRYPTOMAMBA_CONFIG_V2['num_layers']),
            'dropout': float(CRYPTOMAMBA_CONFIG_V2['dropout']),
            'learning_rate': float(CRYPTOMAMBA_CONFIG_V2['learning_rate']),
            'weight_decay': float(CRYPTOMAMBA_CONFIG_V2['weight_decay']),
            'max_epochs': int(CRYPTOMAMBA_CONFIG_V2['num_epochs']),
            'lr_step_size': int(CRYPTOMAMBA_CONFIG_V2['lr_step_size']),
            'lr_gamma': float(CRYPTOMAMBA_CONFIG_V2['lr_gamma']),
            'seq_length': int(CRYPTOMAMBA_CONFIG_V2['seq_length']),
            'use_volume': bool(CRYPTOMAMBA_CONFIG_V2['use_volume']),
        },
        ...
        'baseline_comparison': {
            ...
            'harr_dir_acc': '51.53%',
            'cryptomamba_v2_dir_acc': f"{test_metrics['directional_accuracy']:.2f}%",
    ...
    with open(os.path.join(output_dir, 'cryptomamba_v2_results.json'), 'w') as f:
...
if __name__ == "__main__":
    ...
    print("\n" + "="*80)
    print("CRYPTOMAMBA V2 - FULL ARCHITECTURE")
    print("="*80)
    print("\nObjective: Test if CryptoMamba beats LSTM baseline (original hyperparameters)")
    print("LSTM Performance: 48.01-48.32% Dir Acc (all variants)")
    print("HAR-R Performance: 51.53% Dir Acc")
    print("CryptoMamba V2 Target: >48% (beat LSTM), approach 51% (HAR-R)")
    print("\nKey Improvements:")
    print("  - Learning rate: 0.01 (10× higher)")
    print("  - Max epochs: 1000 (10× longer)")
    print("  - Weight decay: 0.0005 (500× stronger)")
    print("  - Full Mamba architecture (hierarchical)")
    print("="*80 + "\n")

    model, val_metrics, test_metrics = train_cryptomamba_v2(data_dir)
