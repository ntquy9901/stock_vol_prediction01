"""
Enhanced LSTM-HAR Training with 3-Way Temporal Split

This script trains the Enhanced LSTM-HAR model using:
- 70% train data (2006-2020)
- 15% validation data (2020-2021) - for early stopping and model selection
...
def train_enhanced_lstm_har_with_val(data_dir: str, output_dir: str = None):
    """
    Train Enhanced LSTM-HAR with 3-way temporal split (70/15/15).

    Features: [raw parkinson, har weekly, har monthly] (3 features total)

    Args:
        data_dir: Directory containing processed CSV files
        output_dir: Output directory (default: results/enhanced_lstm_har_val_YYYY-MM-DD_HHMMSS)
    """
    if output_dir is None:
        timestamp = datetime.now().strftime("%Y-%m-%d_%H%M%S")
        output_dir = f'results/enhanced_lstm_har_val_{timestamp}'

    print("=" * 80)
    print("ENHANCED LSTM-HAR TRAINING - 3-WAY TEMPORAL SPLIT (70/15/15)")
    print("=" * 80)
    ...
    config = {
        ...
        'dropout': 0.2,              # INCREASED: 0.1 -> 0.2 (prevent overfitting)
    ...
    print("\n4. Initializing Enhanced LSTM-HAR model...")
    ...
    with open(os.path.join(output_dir, 'enhanced_lstm_har_val_results.json'), 'w') as f:
...
def plot_learning_curves(train_losses, val_losses, output_dir, epoch):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss (MSE)')
    plt.title('Enhanced LSTM-HAR Learning Curves (with Validation)')
    plt.legend()
...
if __name__ == "__main__":
    ...
    # Train enhanced LSTM-HAR with validation
